/* global kony */
Object.defineProperty(kony.$kwebfw$, 'Animator', {configurable:false, enumerable:false, writable:false, value:(function() {
    var $K = kony.$kwebfw$, _isPresent = {transform:false, anchorPoint:false}, _vendor = null;


    var _calculateSizeAndUnit = function $K_Anim_calculateSizeAndUnit(value, unit) {
        var $K = kony.$kwebfw$, $KU = $K.utils, data = {value:-1, unit:''};
    ...
    var _createKeyFrames = function $K_Anim_createKeyFrames(widget, name, keyframes, animType, animateHeightOnly) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, _ = widget._kwebfw_, cssRule = '', keyframesSum = {};
        ...
        $KU.each(keyframes, function(stepConfig, step) {
            ...
            $KU.each(keyframesSum, function(value, key) {
                if(!stepConfig.hasOwnProperty(key)) {
    ...
    var _getAnimStyleSheet = function $K_Anim_getAnimStyleSheet() {
        ...
        $KU.each(document.styleSheets, function(sheet) {
            ...
            if(link.tagName === 'LINK' && link.getAttribute('kfwss') === 'kony') {
    ...
    var _style = {
        anchorPoint: function $K_Anim_style_anchorPoint(value) {
            var $K = kony.$kwebfw$, $KU = $K.utils, rule = '';
        ...
        backgroundColor: function $K_Anim_style_backgroundColor(value) {
            var $K = kony.$kwebfw$, $KU = $K.utils, background = '', rule = '';
        ...
        borderColor: function $K_Anim_style_borderColor(value) {
            var $K = kony.$kwebfw$, $KU = $K.utils, rule = '', border = '';
        ...
        bottom: function $K_Anim_style_bottom(value, widget) {
            var $K = kony.$kwebfw$, $KW = $K.widget, $KU = $K.utils, rule = '', key = '',
            ...
            if(layout !== kony.flex.RESPONSIVE_GRID) {
        ...
        left: function $K_Anim_style_left(value, widget) {
            var $K = kony.$kwebfw$, $KW = $K.widget, rule = '',
        ...
        top: function $K_Anim_style_top(value, widget) {
            var $K = kony.$kwebfw$, $KW = $K.widget, $KU = $K.utils, rule = '',
                parent = $KW.pmodel(widget), layout = $KW.layout(parent), msg = '';
    ...
    var animator_animate = function Animator$animate(widget, config, callback, animType, animateHeightOnly) {
        ...
        if(arguments.length === 1) {
            ...
            if($KU.is(config, 'object')) {
                ...
                if([kony.anim.DIRECTION_ALTERNATE].indexOf(config.direction) > 0) {
                ...
                if([kony.anim.FILL_MODE_BACKWARDS, kony.anim.FILL_MODE_BOTH, kony.anim.FILL_MODE_FORWARDS].indexOf(config.fillMode) > 0) {
            ...
            if($KU.is(widget, 'widget', 'Image2') && !_.loaded) {
                ...
                if($KW.isRendered(widget)) {
                    ...
                    $KD.on(animEl, 'animationend', 'anim', function(e) {
                        ...
                        if(config.fillMode === kony.anim.FILL_MODE_FORWARDS
                        || config.fillMode === kony.anim.FILL_MODE_BOTH) {
                            if(config.direction === kony.anim.DIRECTION_NONE) {
                                keyframe = keyframes['100'];
                            } else if(config.direction === kony.anim.DIRECTION_ALTERNATE) {
                                keyframe = (config.iterationCount === 'infinite'
                                            || config.iterationCount % 2 === 0)
                                            ? keyframes['0'] : keyframes['100'];
                            }
                        } else if(config.fillMode === kony.anim.FILL_MODE_BACKWARDS) {
                        ...
                        if(keyframe) {
                            ...
                            $KU.each(keyframe, function(value, key) {
                                ...
                                if(otherProps.indexOf(key) >= 0) {
                                    if(key === 'transform') {
                                        transformObj = new kony.ui.makeAffineTransform();
    ...
    var animator_applyRowAnimation = function Animator$applyRowAnimation (elements, config, callbacks) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, $KW = $K.widget,
            model = null, animName = "anim_" + $KU.uid();
    ...
    $K.defKonyProp(Animator.prototype, [
...
Object.defineProperty(kony.$kwebfw$, 'Transform', {configurable:false, enumerable:false, writable:false, value:(function() {
    var $K = kony.$kwebfw$;
    ...
    var transform_rotate = function Transform$rotate(angle) {
        var $K = kony.$kwebfw$, $KU = $K.utils, rotate = '';
    ...
    $K.defKonyProp(Transform.prototype, [
