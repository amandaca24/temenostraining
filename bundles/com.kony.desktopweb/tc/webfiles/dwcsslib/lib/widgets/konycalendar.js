...
(function() {
    var _days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
        ...
        ], $K = kony.$kwebfw$, _minYear = 1900, _maxYear = 2099;


    $K.defKonyProp($K.ui, [
        {keey:'Calendar', value:{}, items:[
            {keey:'cancelSelection', value:function(evt) {
                var $K = kony.$kwebfw$, $KD = $K.dom, _ = this._kwebfw_, picker = _.picker,
            ...
            {keey:'correctSelection', value:function(evt) {
                var $K = kony.$kwebfw$, $KD = $K.dom,  _ = this._kwebfw_,
                    prop = _.prop, picker = _.picker, dm = _.displayedMonths[0],
                    dc = prop.dateComponents, selectedMonth, selectedYear,
                ...
                if(months) {
                    selectedMonth = $KD.find(months, '[class="'+ skinInfo.selectedValue +'"]')[0];
                    selectedMonth = selectedMonth ? $KD.getAttr(selectedMonth, 'month') : null;
                }
                if(years) {
                    selectedYear = $KD.find(years, '[class="'+ skinInfo.selectedValue +'"]')[0];
                }

                selectedMonth = selectedMonth ? _months.indexOf(selectedMonth) : dm[0];
                selectedYear = selectedYear ? Number($KD.getAttr(selectedYear, 'year')) : dm[1];
            ...
            {keey:'onDateFocus', value:function(evt) {
                ...
                if(prop.viewConfig && prop.viewConfig.gridConfig) {
                    ...
                    selectedSkin = (gridConfig.gridCellSelectedSkin || '-kony-calendar-cell-selected');
                }
            ...
            {keey:'onDateKeyDown', value:function(evt) {
                var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom,
                    prev = null, next = null, i = 0, valid = false, date,
                    code = evt.keyCode || evt.which, targetDate, currentDate,
                    picker, elementClass;
                ...
                if([37, 38, 39, 40].indexOf(code) >= 0) {
                    ...
                    if(code === 37) { //Left Arrow
                        ...
                        if(next && !(elementClass && elementClass === '-kony-clear')) {
            ...
            {keey:'onFocusOut', value:function(evt) {
                ...
                timeout = setTimeout(function() {
                    var $K = kony.$kwebfw$, $KD = $K.dom, _ = self._kwebfw_,
                        dm = _deduceDisplayedMonth.call(self), picker = _.picker;
            ...
            {keey:'onInputKeyDown', value:function(evt) {
                var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom,
                    _ = this._kwebfw_, prop = _.prop,  valid = true,
                    code = evt.keyCode || evt.which, dates = null;
            ...
            {keey:'onNextMonthClick', value:function(evt) {
                var $K = kony.$kwebfw$, $KD = $K.dom, picker = null;
            ...
            {keey:'onYearClick', value:function(evt) {
                ...
                monthCell = $KD.find($KD.find(picker, '[kr="months"]')[0], '[class="'+ currentSkinInfo.selectedValue +'"]')[0];
            ...
            {keey:'onMonthClick', value:function(evt) {
                ...
                yearCell = $KD.find($KD.find(monthYearPicker, '[kr="years"]')[0], '[class="-kony-calendar-cell-selected"]')[0];
    ...
    var _anchorPicker = function Calendar$_anchorPicker() {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom, $KG = $K.globals,
    ...
    var _cellRenderInfo = function Calendar$_cellRenderInfo(date, month, year, displayedMonth, displayedYear) {
        var $K = kony.$kwebfw$, $KU = $K.utils, prop = null, skins = [],
            ...
            from = null, appliedSkinIndex = -1, isDate = {
                getActiveSkin: function(dt, skinConfig) {
                    ...
                    if(!skin) {
                        skin = skinConfig.gridCellSkin || '-kony-calendar-cell';
                    }
                ...
                getWeekendSkin: function (dt, skinConfig) {
                    var key = '', _ = this._kwebfw_,
                        ...
                        skin = skinConfig.gridCellWeekendSkin || '-kony-calendar-cell-weekend';
                ...
                getInactiveSkin: function(dt, skinConfig) {
                    ...
                    if(!skin) {
                        skin = skinConfig.gridCellInactiveDaysSkin || '-kony-calendar-cell-inactive';
                    }
        ...
        if(isDate.today.call(this, date, displayedMonth, displayedYear)) skins.push('gridCellTodaySkin');
        if(isDate.weekend.call(this, date)) skins.push('gridCellWeekendSkin');
        ...
        && isDate.today.call(this, date, displayedMonth, displayedYear)) {
        ...
        info.selectedSkin = gridConfig.gridCellSelectedSkin || '-kony-calendar-cell-selected';
        ...
        if(sd && this._kwebfw_.prop.selectionType === constants.CALENDAR_SELECTION_TYPE_RANGE_SELECT) {
            if(sd.length === 2) {
                ...
                if(date.valueOf() === from.valueOf() && gridConfig.gridCellFromDateSkin) {
                    info.selectedSkin = gridConfig.gridCellFromDateSkin || '-kony-calendar-cell-from';
                } else if(date.valueOf() === to.valueOf() && gridConfig.gridCellToDateSkin) {
                    info.selectedSkin = gridConfig.gridCellToDateSkin || '-kony-calendar-cell-to';
                }
            }
        }
        ...
        if(info.skinType === 'gridCellSpecialSkin') {
            ...
            info.skin = gridConfig.gridCellTodaySkin || '-kony-calendar-cell-today';
    ...
    var _closePicker = function Calendar$_closePicker(focus) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KD = $K.dom,
            _ = this._kwebfw_, parent = null, fmodel = null, el = null;
        ...
        if($KU.is(_.picker, 'dom')) {
            ...
            if(fmodel._kwebfw_.pickers) {
                delete fmodel._kwebfw_.pickers[_.uid];
            }
    ...
    var _deduceDisplayedMonth = function Calendar$_deduceDisplayedMonth() {
        var $K = kony.$kwebfw$, $KU = $K.utils, date = new Date(),
    ...
    var _dismissPicker = function Calendar$_dismissPicker(focus) {
        var $K = kony.$kwebfw$, $KU = $K.utils, _ = this._kwebfw_, dm = null;
    ...
    var _formatDate = function Calendar$_formatDate() {
        var $K = kony.$kwebfw$, $KU = $K.utils, prop = this._kwebfw_.prop, result = '';
    ...
    var _getAllSelectedDates = function Calendar$_getAllSelectedDates() {
        var $K = kony.$kwebfw$, $KU = $K.utils, _ = this._kwebfw_, dates = [];
    ...
    var _getSkinRelatedSkinInfoByGridCell = function Calendar$_getSkinRelatedSkinInfoByGridCell(cell) {
        var $K = kony.$kwebfw$, $KD = $K.dom, info = {},
    ...
    var _getter = {
        Calendar: {
            ...
            selectedDates: function Calendar$_getter_selectedDates(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, dates = [];
    ...
    var _getMonthYearHeader = function() {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, _ = this._kwebfw_,
                ...
                header, headerSkin = '-kony-calendar-header', month, year, title, row, labelCell,
            ...
            if(prop.gridTheme === kony.calendar.MODERN) {
                headerSkin = (viewConfig && viewConfig.headerSkin)
                            ? viewConfig.headerSkin : '-kony-calendar-header';
    ...
    var _getMonths = function() {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, size = _months.length,
            monthString = "", monthsArray = _months.slice(1, size), monthsBlock,
            row, cell, html = '',  _ = this._kwebfw_, prop = _.prop,
            viewConfig = prop.viewConfig ? prop.viewConfig.gridConfig : null,
            cellSelectedSkin = '-kony-calendar-cell-selected';
        ...
        $KU.each(monthsArray, function(block) {
            ...
            $KU.each(block, function(month) {
                ...
                $KD.setAttr(cell, 'cellskinvalue', '-kony-calendar-cell');
                $KD.setAttr(cell, 'cellselectedskin', cellSelectedSkin);
                $KD.addCls(cell, '-kony-calendar-cell');
    ...
    var _getYears = function(firstYear) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom,
            _ = this._kwebfw_, prop = _.prop, i, html = '', counter, row,
            dc = prop.dateComponents, cell, yearBlock,  _ = this._kwebfw_,
            prop = _.prop, viewConfig = prop.viewConfig ? prop.viewConfig.gridConfig : null,
            cellSelectedSkin = '-kony-calendar-cell-selected';
    ...
    var _isDateInactive = function Calendar$_isDateInactive(dc) {
        var $K = kony.$kwebfw$, $KU = $K.utils, dt = null,
    ...
    var _maxDate = function Calendar$_maxDate(month, year) {
        var $K = kony.$kwebfw$, $KU = $K.utils, max = -1;
    ...
    var _onDateEdited = function Calendar$_onDateEdited(formattedDate) {
        var $K = kony.$kwebfw$, $KW = $K.widget, dates = null,
            _ = this._kwebfw_, prop = _.prop, valid = true;
    ...
    var _openPicker = function Calendar$_openPicker() {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom, picker = null,
            fmodel = null, el = $KW.el(this), _ = this._kwebfw_, prop = _.prop,
        ...
        fmodel._kwebfw_.pickers[_.uid] = _.uid;
        ...
        Calendar: function Calendar$_populateUnderscore() {
            ...
            if(!_.ns) {
                if($KU.is(this.__$kwebfw$ns__, 'string') && this.__$kwebfw$ns__) {
                    ...
                    $KU.defineProperty(_, 'ns', 'kony.ui.Calendar', null);
                }
            }
            if(!_.name) {
    ...
    var _prepareDates = function Calendar$_prepareDates(month, year, startingDay) {
        var $K = kony.$kwebfw$, $KU = $K.utils, c = 0, prevYear = year,
        ...
        && _weekDays.hasOwnProperty(startingDay)) {
    ...
    var _prepareDays = function Calendar$_prepareDays(startingDay) {
        var $K = kony.$kwebfw$, $KU = $K.utils, c = 0, days = [];
    ...
    var _renderCalendar = function Calendar$_renderCalendar(month, year, date) {
        var $K = kony.$kwebfw$, $KD = $K.dom, calendar = null, size = null, calheight = 0,
            _ = this._kwebfw_, prop = _.prop, viewConfig = prop.viewConfig,
            gridConfig = (viewConfig && viewConfig.gridConfig) ? viewConfig.gridConfig : {};

        size = _deduceCellHeightAndWidth.call(this);
        if(prop.gridTheme === kony.calendar.LEGACY) {
        ...
        $KD.addCls(calendar, (gridConfig.gridSkin || '-kony-calendar-grid'));
    ...
    var _renderDates = function Calendar$_renderDates(month, year, startingDay, weekends) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, grid = null,
        ...
        $KU.each(dates, function(date) {
            var $K = kony.$kwebfw$, $KD = $K.dom, info = null,
        ...
        $KD.add(row, $KD.create('DIV', {class:'-kony-clear'}));
        $KD.add(grid, row);
    ...
    var _renderDays = function Calendar$_renderDays(startingDay, weekends) {
        ...
        $KU.each(days, function(day) {
            ...
            //$KD.addCls(cell, 'textAlign', ('-kony-ca-'+this.dayTextAlignmentInCell));
            $KD.html(cell, _days[day].toUpperCase());
            $KD.add(wrapper, cell);
        }, this);

        $KD.add(wrapper, $KD.create('DIV', {class:'-kony-clear'}));
    ...
    var _renderMonthYearDropDownHeader = function Calendar$_renderMonthYearDropDownHeader(month, year) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, header = null,  _ = this._kwebfw_,
            prop = _.prop, viewConfig = prop.viewConfig ? prop.viewConfig.gridConfig : null,
            row = null, prevMonthCell = null, nextMonthCell = null, prevMonthSrc = '',
            nextMonthSrc = '', monthHolder = null, yearHolder = null, selectedSkin = '-kony-calendar-cell-selected',
            hoverSkin = '-kony-calendar-cell-hover', headerSkin = '-kony-calendar-header';

        if(viewConfig) {
            selectedSkin = viewConfig.gridMonthYearSelectedSkin || '-kony-calendar-cell-selected';
            hoverSkin = viewConfig.gridMonthYearHoverSkin || '-kony-calendar-cell-hover';
        }
    ...
    var _renderMonthYearPopupHeader = function Calendar$_renderMonthYearPopHeader(month, year) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, header = null, _ = this._kwebfw_,
            ...
            nextYearSrc = '', headerSkin = '-kony-calendar-header';
    ...
    var _renderSelectedMonthYear = function Calendar$_renderSelectedMonthYear(selectedOption) {
        var $K = kony.$kwebfw$, $KD = $K.dom, _ = this._kwebfw_, displayedMonths = _.displayedMonths[0],
    ...
    var _renderPicker = function Calendar$_renderPicker(date) {
        ...
        if(prop.gridTheme === kony.calendar.LEGACY) {/*grid*//* header *//* weekdays *//*footer*//*unknown*/
    ...
    var _renderMonthYear = function() {
        ...
        if(prop.gridTheme === kony.calendar.LEGACY) {// -10px is to balance height with extra height because of header padding
        ...
        $KD.addCls(MonthYearPicker, ((viewConfig && viewConfig.gridSkin) ? viewConfig.gridSkin : '-kony-calendar-grid'));
    ...
    var _renderTemplate = function Calendar$_renderTemplate(date) {
        ...
        if(prop.data && prop.widgetDataMapForCell) {
            ...
            if(cellData) {
                ...
                if($KU.is(tpl, 'string')) {
                    $K.app.allowSetter_konyControllerName = true;
                    tpl = _kony.mvc.initializeSubViewController(tpl);
                    delete $K.app.allowSetter_konyControllerName;
    ...
    var _selectDateFromPicker = function Calendar$_selectDateFromPicker(cell) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, prevSkinInfo = null, fdate = null,
        ...
        if(this.selectionType === constants.CALENDAR_SELECTION_TYPE_SINGLE_SELECT) {
            ...
            if($KD.hasCls(cell, currentSkinInfo.selectedValue)) {
                ...
                if(pos !== -1) _.selectedDates.splice(pos, 1);
    ...
    var _showMonthYearPicker = function() {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, month, year, labelCell,
            _ = this._kwebfw_, prop = _.prop, picker = _.picker, cal, py, cy,
            holder, dm = _.displayedMonths[0], dc = prop.dateComponents, yearBlock = null;
            monthYearPicker = $KD.find(picker, '[kr="monthYearPicker"]')[0];
    ...
    var _toggleRangeOfCells = function Calendar$_toggleRangeOfCells(picker, from, to, select) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, fstr = '', fdate = null,
    ...
    var _showNextYearSet = function() {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, _ = this._kwebfw_,
            prop = _.prop, counter, yearsEl, yearBlock, picker;
        ...
        Calendar: {
            ...
            dateComponents: function Calendar$_valid_dateComponents(value) {
                ...
                if(flag === true || ($KU.is(flag, 'array') && flag[1] === true)) {
                    ...
                    if(date) {
                        if(prop.hasOwnProperty('validStartDate')
                        && prop.hasOwnProperty('validEndDate')
                        && prop.hasOwnProperty('selectionType')
                        && prop.hasOwnProperty('selectedDates')) {
                            dc = prop.dateComponents;
            ...
            gridTheme: function Calendar$_valid_gridTheme(value) {
                var flag = false, options = [
                    kony.calendar.LEGACY,
                    kony.calendar.MODERN
                ];
            ...
            selectedDates: function Calendar$_valid_selectedDates(value) {
                ...
                if($KU.is(value, 'null')) {
                    ...
                    if(flag) {
                        ...
                        && prop.hasOwnProperty('validEndDate')
                        && prop.hasOwnProperty('selectionType')
                        && prop.hasOwnProperty('dateComponents')) {
                            if(prop.selectionType === constants.CALENDAR_SELECTION_TYPE_MULTI_SELECT) {
            ...
            validEndDate: function Calendar$_valid_validEndDate(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, endDate = null,
            ...
            validStartDate: function Calendar$_valid_validStartDate(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, startDate = null,
                    flag = _isValidDateComponents(value), prop = this._kwebfw_.prop,
                ...
                if(flag === true || ($KU.is(flag, 'array') && flag[1] === true)) {
                    ...
                    if(startDate) {
                        if(prop.hasOwnProperty('validEndDate')
                        && prop.hasOwnProperty('selectionType')
                        && prop.hasOwnProperty('dateComponents')
                        && prop.hasOwnProperty('selectedDates')) {
            ...
            viewConfig: function Calendar$_valid_viewConfig(value) {
                ...
                if($KU.is(value, 'null')) {
                    ...
                    if(!value.hasOwnProperty('gridConfig')) {
                        flag = true;
                    } else {
    ...
    var _view = {
        Calendar: {
            ...
            dateFormat: function Calendar$_view_dateFormat(el, old) {
                var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom, el = $KW.el(this),
            ...
            enableOrDisableDates: function Calendar$_view_enableOrDisableDates(el, old) {
                var $K = kony.$kwebfw$, $KU = $K.utils, _ = this._kwebfw_, dc = _.prop.dateComponents,
                    ...
                    if($KU.is(skin, 'string') && $KU.is(enable, 'boolean')) {
                        $KU.each(dates, function(date) {
                            ...
                            if(dc && $KU.is(date, 'date')) {
                                ...
                                dc = new Date(dc[2], (dc[1] - 1), dc[0], 0, 0, 0);
    ...
    Object.defineProperty(kony.ui, 'Calendar', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.Calendar constructor.
         *
         * @class
         * @namespace   kony.ui
        ...
        var Calendar = function Calendar(bconfig, lconfig, pspconfig) {
            ...
            if(!bconfig.isPreValidated) {
                prop = {
                    ...
                    gridTheme: kony.calendar.MODERN, //(Read + Write)
                ...
                if(!bconfig.isPreValidated) {
                    ...
                    $KU.each(prop, function (value, key) {
                        ...
                        if(!bconfig.hasOwnProperty(key)) {
                            ...
                            throw new Error('<' + key + '> is available in default widget properties of <kony.ui.Calendar>, but not in <_valid.Calendar> namespace.');
                        } else {
        ...
        $K.utils.inherits(Calendar, kony.ui.BasicWidget);

        var calendar__flush = function Calendar$_flush() {
            var $super = kony.ui.Calendar.base.prototype,
                $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom, el = $KW.el(this);
        ...
        /**
         * Builds the view layer for kony.ui.Calendar widget.
         *
         * @override
         * @access      protected
         * @method      _render
         * @memberof    kony.ui.Calendar
         * @author      Goutam Sahu <goutam.sahu@kony.com>
         *
         * @returns     {HTMLElement} – Calendar view.
         */
        var calendar__render = function Calendar$_render(tag) {
            var $super = kony.ui.Calendar.base.prototype, icon = null,
                _ = this._kwebfw_, prop = _.prop, input = null, view = _.view,
                $K = kony.$kwebfw$, $KD = $K.dom, $KW = $K.widget, el = $KW.el(view);
        ...
        var calendar_removeDataAt = function Calendar$removeDataAt(date) {
            ...
            if(prop.data && prop.data.hasOwnProperty(date)) {
        ...
        var calendar_setDatesSkin = function Calendar$setDatesSkin(dates, skin) {
            var $K = kony.$kwebfw$, $KU = $K.utils, _ = this._kwebfw_, valid = true;
        ...
        var calendar_setEnabled = function Calendar$setEnabled(dates, skin, enable) {
            ...
            if(arguments.length === 1) {
                if($KU.is(dates, 'boolean')) {
                    kony.ui.Calendar.base.prototype.setEnabled.call(this, dates);
                }
        ...
        var calendar_setEnableAll = function Calendar$setEnableAll(arg0) {
            var _ = this._kwebfw_, enable = (arg0 === false) ? false : true;

            this.enableRangeOfDates([1, 1, _minYear], [31, 12, _maxYear], '', enable);
        };

        $K.defKonyProp(Calendar.prototype, [
