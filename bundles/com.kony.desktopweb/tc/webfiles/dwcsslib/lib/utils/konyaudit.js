/* global kony */
Object.defineProperty(kony.$kwebfw$, 'audit', {configurable:false, enumerable:false, writable:false, value:(function() {
    ...
    function _getForms() {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KU = $K.utils, forms = [];

        $KU.each($KW.model(), function(value) {
            if($KW.root(value)
            && value._kwebfw_.is.template !== true
            && value instanceof kony.ui.Form2) {
                forms.push(value);
            }
        });
    ...
    function _getTemplates() {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KU = $K.utils, templates = [];
        ...
        $KU.each($KW.model(), function(value) {
            ...
            if($KW.root(value)
            && value._kwebfw_.is.template === true
            && value instanceof kony.ui.ContainerWidget
            && !(value instanceof kony.ui.FlexScrollContainer)
            && !(value instanceof kony.ui.Form2)) {
    ...
    function _getUIDsByID(arg0) {
        var $K = kony.$kwebfw$, $KU = $K.utils, ids = [],
    ...
    function _getWidgetDetails(widgets, arg1) {
        ...
        $KU.each(widgets, function(widget) {
            var $K = kony.$kwebfw$, $KW = $K.widget, detail = {},
    ...
    function _getWidgetsByID(id, type, autoHeight, autoWidth, filterHandler) {
        var $K = kony.$kwebfw$, $KU = $K.utils, ret = {};
    ...
    function _getWidgetsByRootID(id, type, autoHeight, autoWidth, filterHandler) {
        var $K = kony.$kwebfw$, $KU = $K.utils, roots = [], ret = {};
    ...
    function _getWidgetsByUID(uid, type, autoHeight, autoWidth, filterHandler) {
        var $K = kony.$kwebfw$, $KU = $K.utils, widgets = [],
        ...
        filterHandler = ($KU.is(filterHandler, 'function')) ? filterHandler : function() {return true;};
