...
(function(tcc) {
    ...
    tcc.generateCSSForSPA = function(cgArgs) {
        ...
        if(fs.existsSync(outputdir)) {
            try {
                ...
                if(themeJSON['theme_styles']) {
                    cgArgs.theme_constants = themeJSON.theme_styles;
                    delete themeJSON.theme_styles;
                } else {
                    cgArgs.theme_constants = null;
                }
            } catch (e) {
    ...
    function renderCommonCSSForWidgets() {
        var widgetMarginsBuffer = new konyUtil.StringBuffer();
        var templateFile = velocitytemplatesmap['spa']["containerskincss"];
        var result = konyUtil.templaterenderer(templateFile, {});
    ...
    function generateIPhoneCSS(themeJSON, themeName, cgArgs) {
        try{
            var iphoneRetinaCSSCodeBuffer = new konyUtil.StringBuffer();
            var platform = cgArgs.platform;
            ...
            for(var i in formFactors) {
                ...
                var iphoneCSSCodeBuffer = new konyUtil.StringBuffer();
                var result = "";
                ...
                if(themeName != "default") {
                    ...
                    konyUtil.assertDirectory(destDir, themeName);
                    destDir = path.resolve(destDir, themeName);
                }
                var cssFileName = konyUtil.getCSSFileName(platform, formFactor);
                var result = beautify_css(iphoneCSSCodeBuffer.toString());
                konyUtil.writeToFile(destDir, cssFileName, result);
    ...
    function generateAndroidCSS(themeJSON, themeName, cgArgs) {
        ...
        try {
            for(var i = 0 ; i < droidFormFactors.length; i++) {
                var androidCSSCodeBuffer = new konyUtil.StringBuffer();
    ...
    function generateBBCSS(themeJSON, themeName, cgArgs) {
        ...
        try {
            for(var i = 0 ; i < BBFormFactors.length; i++) {
                var bbCSSCodeBuffer = new konyUtil.StringBuffer();
    ...
    function generateADVIE10CSS(themeJSON, themeName, cgArgs) {
        ...
        try {
            for(var i = 0 ; i < IEFormFactors.length; i++) {
                var ie10CSSCodeBuffer = new konyUtil.StringBuffer();
    ...
    function generateWindowsTabCSS(themeJSON, themeName, cgArgs) {
        ...
        try{
            var formFactor = "";
            var windowsTabCSSCodeBuffer = new konyUtil.StringBuffer();
    ...
    function generateDesktopwebCommonCSS(themeJSON, themeName, cgArgs) {
        ...
        try {
            var formFactor = "";
            var desktopCSSCodeBuffer = new konyUtil.StringBuffer();
    ...
    function generateDesktopwebLibCSS(themeJSON, themeName, cgArgs) {
        ...
        try {
            ...
            destDir = cgArgs.outputdir;
            konyUtil.assertDirectory(destDir, 'kwebthemes');
