/* global kony */
Object.defineProperty(kony, 'media', {configurable:false, enumerable:false, writable:false, value:(function() {
    var _ns = {}, $K = kony.$kwebfw$;

    var _record = function(config, arg1) {
        var $K = kony.$kwebfw$, $KU = $K.utils, _audioRecorder = null,
            _recordMedia = null, tracks = null;

        $KU.log({api:'kony.media.record', enter:true});
        ...
        $KU.log({api:'kony.media.record', exit:true});
        return _recordMedia;
    };
...
/* global kony */
Object.defineProperty(kony, 'screenrecorder', {configurable:false, enumerable:false, writable:false, value:(function() {
    var _ns = {}, $K = kony.$kwebfw$, _ScreenRecorder = [], _videoData = [], _tracks = [];

    var _start = function(config) {
        var $K = kony.$kwebfw$, $KU = $K.utils, frameRate = 30,
            constraints = null, videoTracks = null;

        $KU.log({api:'kony.screenrecorder.start', enter:true});
        ...
        if($KU.browser('supports', 'displaymedia') && $KU.browser('supports', 'mediarecorder')) {
            navigator.mediaDevices.getDisplayMedia(constraints).then(function(stream) {
                ...
                $KU.log({api:'kony.screenrecorder.start', exit:true});
            }).catch( function(err) {
    ...
    var _stop = function() {
        ...
        if(_ScreenRecorder && !(_ScreenRecorder.state === 'inactive')) {
            $KU.log({api:'kony.screenrecorder.stop', enter:true});
            ...
            $KU.log({api:'kony.screenrecorder.stop', exit:true});
        }
    };
    ...
    var _pause = function() {
        ...
        if(_ScreenRecorder && _ScreenRecorder.state === 'recording') {
            $KU.log({api:'kony.screenrecorder.pause', enter:true});
            _ScreenRecorder.pause();
            $KU.log({api:'kony.screenrecorder.pause', exit:true});
        }
    };
    ...
    var _resume = function() {
        ...
        if(_ScreenRecorder && _ScreenRecorder.state === 'paused') {
            $KU.log({api:'kony.screenrecorder.resume', enter:true});
            _ScreenRecorder.resume();
            $KU.log({api:'kony.screenrecorder.resume', exit:true});
        }
    };

    var _getrecordeddata = function() {
        var $K = kony.$kwebfw$, $KU = $K.utils, blob = null, videoBlobURL = null;
        if(_ScreenRecorder && _ScreenRecorder.state && _ScreenRecorder.state === 'inactive') {
            $KU.log({api:'kony.screenrecorder.getrecordeddata', enter:true});
            blob = new Blob(_videoData, { type:'video/webm' });
            window.URL = window.URL || window.webkitURL;
            videoBlobURL = window.URL.createObjectURL(blob);
            $KU.log({api:'kony.screenrecorder.getrecordeddata', exit:true});
        }
        return videoBlobURL;
    };

    var _getrecordingstate = function() {
        var $K = kony.$kwebfw$, $KU = $K.utils, state = 'inactive';
        $KU.log({api:'kony.screenrecorder.getrecordedstate', enter:true});
        if(_ScreenRecorder && _ScreenRecorder.state) {
            $KU.log({api:'kony.screenrecorder.getrecordedstate', exit:true});
