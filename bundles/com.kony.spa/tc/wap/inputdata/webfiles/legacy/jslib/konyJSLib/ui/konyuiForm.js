        ...
        if(!this._konyControllerName) {
            defineGetter(window, id, function() {
                if($KC.widgetDataRecording == true &&
                    ...
                    $KC.konyCOPFormAccess[that.id] = true;
                }
...
kony.ui.Form2.prototype._show = function() {
    if($KC.widgetDataRecording == true) {
        var key;
        if($KC.konyCOPFormAccess) {
            if($KC.konyCOPFormAccess[this.id]) {
                delete $KC.konyCOPFormAccess[this.id];
            }

            for(key in $KC.konyCOPFormAccess) {
                $KC.konyCOPPrerenderFormAccessedData[key] = true;
            }

            $KC.konyCOPFormAccess = {};

            if(typeof $KC.konyCOPPrerenderFormAccessedData[this.id] === 'undefined') {
                $KC.konyCOPPrerenderFormAccessedData[this.id] = false;
            }
        }
