...
$KC = (function() {
    ...
    var _addInlineProperties = function(widgetModel, formID) {
        ...
        for(key in _inlinePropertyMap) {
            if(!inlineProperties[_inlinePropertyMap[key]] && typeof _inlinePropertyMap[key] !== "undefined") {
                if(_inlinePropertyMap[key] === 'Type' &&
                   widgetModel.masterType === constants.MASTER_TYPE_DEFAULT || widgetModel.name === "kony.ui.KComponent") {
                    inlineProperties[_inlinePropertyMap[key]] = 'kony.ui.User';
                } else {
        ...
        if(widgetSpecificProperties) {
            for(key in widgetSpecificProperties) {
                ...
                if(key === "layoutType") {
                    if(value == kony.flex.FLOW_HORIZONTAL) {
                        value = "FLOW_HORIZONTAL";
                    } else if(value == kony.flex.FLOW_VERTICAL) {
                        value = "FLOW_VERTICAL";
                    } else {
                        value = "FREE_FORM";
                    }
        ...
        for(key in _dataSet) {
            if($KG.appbehaviors["responsive"] === true)  {
                for(subKey in _dataSet[key]) {
                    _dataSet[key][subKey].inlineProperties.FormLoadedBeforeShow = $KC.konyCOPPrerenderFormAccessedData[key];
                    if($KC.konyCOPFormAccess && $KC.konyCOPFormAccess[key]) {
                        _dataSet[key][subKey].inlineProperties.FormLoadedBeforeShow = $KC.konyCOPFormAccess[key];
                    }
                }
            } else {
                _dataSet[key].inlineProperties.FormLoadedBeforeShow = $KC.konyCOPPrerenderFormAccessedData[key];
                if($KC.konyCOPFormAccess && $KC.konyCOPFormAccess[key]) {
                    _dataSet[key].inlineProperties.FormLoadedBeforeShow = $KC.konyCOPFormAccess[key];
                }
            }
        }
    };
    ...
    var module = {
        konyCOPFormAccess : {},
        konyCOPPrerenderFormAccessedData : {},
        ...
        generateXMLFormJSON: function() {
            ...
            if(kony.appinit.isMob) channel = "MOBILE";
            else if(kony.appinit.isTablet) channel = "TABLET";
