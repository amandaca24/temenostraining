...
$KU = kony.utils = {
    ...
    getMicroAppName: function() {
        return _kony.mvc.getCurrentAppName();
    },

    getFormInfoFromURL: function() {
        var hash = location.hash, appName = '', formId = '';
        hash = hash.substr(kony.events.browserback.HASH_LENGTH);
    ...
    getParentComponentController: function(widgetModel) {
        ...
        controller = _kony.mvc.ctrlname2ControllerMap[container._konyControllerName];
    ...
    getPlatformDeviceOS: function() {
        ...
        if(navigator.userAgent.indexOf('Windows NT') >= 0 && !kony.appinit.isWindowsTablet) {
            return constants.DEVICE_OSNAME_WINDOWS;
        } else if (kony.appinit.isWindowsTablet) {
        ...
        if(kony.appinit.isBlackBerryNTH) {
            return constants.DEVICE_OSNAME_BLACKBERRYNTH;
        }
        if(kony.appinit.isBlackBerry || kony.appinit.isPlaybook) {
    ...
    getRelativeURL: function(src, path) {
        ...
        return kony.appinit.getStaticContentPath() + platformver + path + src;
    },
    ...
    DeepCopyAlgorithm.prototype = {
        ...
        applyDeepCopier: function(deepCopier, source) {
            ...
            if(source.isMaster && source.__bconfig && source.parent) {
                ...
                setController = true;
                source =  kony.ui.KMasterTemplate(source.__bconfig, {}, {}, source.className);
                this.copiedObjects.pop();
                this.cacheResult(source, result);
                _konyConstNS.ContainerWidget.prototype.removeAt.call(parent, index);
                _konyConstNS.ContainerWidget.prototype.addAt.call(parent, source, index);

                if(templateModel) {
                    _konyConstNS.Form2.prototype.createFormLevelHierarchy.call(templateModel, templateModel.ownchildrenref);
                } else {
                    throw "template model is not there";
                }

                if(!kony.enableGettersSetters) {
            ...
            if(setController) {
                var controller = _kony.mvc.getComponentController(result._konyControllerName, result.appName);
