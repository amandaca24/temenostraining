...
/* global Event */
/* global Touch */
Object.defineProperty(kony.$kwebfw$, 'dom', {configurable:false, enumerable:false, writable:false, value:(function() {
    var _ns = {}, $K = kony.$kwebfw$, __event = {}, _keyCodeMap = {
        "8":  'Backspace',
        "9":  'Tab',
        "13": 'Enter',
        "16": 'ShiftLeft',
    ...
    var _addEvent = function $KD_addEvent(el, type, name, cb, config) {
        ...
        if(!config.hasOwnProperty('scope')) config.scope = el;
    ...
    var _addGesture = function $KD_addGesture(el, type, name, cb, config) {
        ...
        && ($KU.is(name, 'string') && name) && ($KU.is(cb, 'function'))) {
            ...
            if(!config.hasOwnProperty('scope')) config.scope = el;
            if(!config.hasOwnProperty('boundary')) config.boundary = document;
    ...
    var _gesture = {
        ...
        pan: {
            add: function(el, name, cb, config) {
                _addGesture(el, 'pan', name, function(g) {
                    var $K = kony.$kwebfw$, $KU = $K.utils, scope = g.scope;
        ...
        pinch: {
            add: function(el, name, cb, config) {
                _addGesture(el, 'pinch', name, function(g) {
                    var $K = kony.$kwebfw$, $KU = $K.utils, scope = g.scope;
        ...
        rotation: {
            add: function(el, name, cb, config) {
                _addGesture(el, 'rotation', name, function(g) {
                    var $K = kony.$kwebfw$, $KU = $K.utils, scope = g.scope;
                ...
                _addGesture(el, 'tap', name, function(g) {
                    ...
                    if(g.status === 'ended' && $KU.is(cb, 'function')) {
                        if(g.event.start.target === g.event.end.target) {
                            ...
                            //kony.print('g.event.start.target !== g.event.end.target');
                        }
                    }
                }, config);
            }, remove: function(el, name, name) {
    ...
    var _removeEvent = function $KD_removeEvent(el, type, name) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, eid = '', emap = null;
    ...
    var _closest = function $KD_closest(el, name, value) {
        var $K = kony.$kwebfw$, $KU = $K.utils, closest = null;
    ...
    var _create = function $KD_create(tag, attr, style) {
        var $K = kony.$kwebfw$, $KU = $K.utils, fragment = null;

        if(arguments.length > 0) {
            if(arguments.length === 1) {attr = {}; style = {};}
    ...
    var _keyCode = function $KD_keyCode(evt) {
        var $K = kony.$kwebfw$, $KU = $K.utils, code = null;

        if(evt instanceof Event) {
            if(evt.hasOwnProperty('code')) {
                code = evt.code;
            } else if($KU.is(evt.keyCode, 'number')
            && _keyCodeMap.hasOwnProperty(evt.keyCode.toString())) {
        ...
        if(($KU.is(el, 'dom') || el === document || el === window)) {
            if(($KU.is(types, 'string') && types) || $KU.is(types, 'array')) {
                ...
                if($KU.is(types, 'array')) {
                    $KU.each(types, function(value, index) {
    ...
    var _style = function $KD_style(el, key, value) {
        var $K = kony.$kwebfw$, $KU = $K.utils, style = null;
