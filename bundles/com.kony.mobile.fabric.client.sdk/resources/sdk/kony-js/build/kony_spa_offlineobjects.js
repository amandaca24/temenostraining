/**
 * kony-spa-offlineobjects version 9.3.1
    ...
    var KSDatabaseAPI = /** @class */ (function () {
            ...
            KSDatabaseAPI.initializeDatabase = async function (dbname, nextIndexedDBVersion, dbInfo) {
                try {
                    ...
                    dbInstance = await KNYNoSQLDatabaseHelper.getDBInstance(dbname, nextIndexedDBVersion, dbInfo);
                } catch (DBException) {
            ...
            KSDatabaseAPI.getCurrentDBVersion = async function (databaseName) {
                return await KNYNoSQLDatabaseHelper.getCurrentDBVersion(databaseName);
            };
...
define("KSDatabaseUpgradeManager",
    ...
    function (exports, _KSDatabaseAPI, KSCommonUtils, KSQueryObjectBuilder) {
        ...
        function getQueriesToPerformDBUpgradeFromV04ToV05() {
            ...
            shouldUpdatePropertiesTable = true;
            kony.store.removeItem(DB_SCHEMA_VERSION);

            nextIndexedDBVersion = KSDatabaseSchemaVersion.Version_2;
        }

        /**
         * Method to fetch current DB Schema version from konysyncProperties Table
         *
         * @return dbSchemaVersion
         */
        async function getExistingDBSchemaVersion() {
            logger.trace(LOG_PREFIX, "Fetching existing DB schema version from konysyncPROPERTIES table.");
...
define("KSSyncDatabaseHelper", ["exports", "KSDatabaseAPI", "KSSQLQueryGenerator", "KSCommonUtils", "KSSDKObjectRecord",
    ...
    function (exports, _KSDatabaseAPI, KSSQLQueryGenerator, KSCommonUtils, _KSSDKObjectRecord,
        ...
        var MatchType = kony.sdk.constants.MatchType;
        var KSDatabaseAPI = _KSDatabaseAPI.KSDatabaseAPI;
            ...
            try {
                for ( var i in sdkObjectRecords) {
                    ...
                    if (selectedRecords.length > 0) {
                        ...
                        recordsToParse.remove(sdkObjectRecords[i]);
                    }
                }
            } catch (err) {
        ...
        async function persistUploadResponseForObjects(sdkObjects) {
            ...
            for (var index = 0; index < sdkObjectsLen; index++) {
                ...
                for (var primaryKeyValueMap in uploadResponseFilterMap) {
                    ...
                    if (!kony.sdk.isNullOrUndefined(latestSuccessRecord) && latestSuccessRecordRowId !== -1) {
                        kony.sdk.logsdk.debug("Filtered the records with given primary-key value " + primaryKeyValueMap +
        ...
        /**
         * In hierarchy flow, this method is used to get legal parent records for particular action (kony sync change type)
        ...
        async function readRecordsFromHistoryTableOfRootParentObject(
            ...
            const operator = shouldGetPendingRecords ? kony.nosql.GTE : kony.nosql.LT;
        ...
        /**
         * In hierarchy flow, this method is used to get non-pending child records for particular action (kony sync change type)
         * @param sdkObject the object for which records are to be fetched
         * @param action the action type of objects to be fetched<KSSDKObjectRecordAction>
        ...
        async function getMinReplaySequenceNumbersOfParentRecords(
            ...
            var operator = shouldGetPendingRecords ? kony.nosql.GTE : kony.nosql.LT;
        ...
        function isRecordCoRelated(
            ...
            var KSDatabaseConstants = kony.sdk.OfflineObjects.KSDatabaseConstants;
            var canPick = comparisonOperator === kony.nosql.LT;
        ...
        function configureActionAndCheckSumForDatabaseRecord(record) {
            ...
            if (!KSCommonUtils.isNullOrEmptyObject(actionCode)) {
                var actionString = kony.sdk.OfflineObjects.getValueOfRecordActionAsString[actionCode];
...
define("KonyNoSQLDatabaseHelper", ["require", "exports", "KSCommonUtils", "KSError", "KSExceptionWrapperUtils"], function (require, exports, KSCommonUtils, _KSError, KSExceptionWrapperUtils) {
    ...
    var KonyNoSQLDatabaseHelper = /** @class */ (function () {
        ...
        function getRuleFromRuleArray(ruleArray) {
            ...
            var rule = getKonyNoSqlRule(lhsOperand, operation, rhsOperand);
            log.trace(LOG_PREFIX + '[getKonyNoSqlConditionFromArray] formed a rule - lhs:' +
        ...
        function getKonyNoSqlConditionFromArray(whereClauseArray) {
            log.trace(
                LOG_PREFIX + '[getKonyNoSqlConditionFromArray] being called');
            var whereCondition = null;
            var conditionLength = whereClauseArray.length;

            if (conditionLength >= 1) {
                var firstRuleArray = whereClauseArray[0];
                log.trace(LOG_PREFIX + '[getKonyNoSqlConditionFromArray] processing first rule');
                if (isRuleArray(firstRuleArray)) {
                    log.trace(LOG_PREFIX
                              + '[getKonyNoSqlConditionFromArray] Creating a simple Condition with only one rule');
                    whereCondition = new kony.nosql.Condition(getRuleFromRuleArray(firstRuleArray));
                } else {
                    log.trace(LOG_PREFIX + '[getKonyNoSqlConditionFromArray] Found nested rule');
                    whereCondition = new kony.nosql.Condition(getKonyNoSqlConditionFromArray(firstRuleArray));
                }
            }

            for (var outerIndex = 1; outerIndex < conditionLength; outerIndex += 2) {
                var conjunctionOperation = whereClauseArray[outerIndex][0];
                var conjunctionOperator = getLogicalKonyOperation(conjunctionOperation);
                var nextRuleArray = whereClauseArray[outerIndex + 1];
                log.trace(LOG_PREFIX
                          + '[getKonyNoSqlConditionFromArray] Adding next Rule of the complex array to the'
                          + ' existing where condition' + whereCondition.toString());

                if (isRuleArray(nextRuleArray) === true) {
                    log.trace(LOG_PREFIX
                              + '[getKonyNoSqlConditionFromArray] Appending simple next Rule to existing condition');
                    whereCondition.addRule(conjunctionOperator, getRuleFromRuleArray(nextRuleArray));
                } else {
                    log.trace(LOG_PREFIX
                              + '[getKonyNoSqlConditionFromArray] Appending nested next Condition to existing'
                              + ' condition');
                    whereCondition.addCondition(conjunctionOperator,
                                                getKonyNoSqlConditionFromArray(nextRuleArray));
                }
            }

            var conditionString = null;
            if (!kony.sdk.isNullOrUndefined(whereCondition)) {
                conditionString = whereCondition.toString();
            }
            log.trace(LOG_PREFIX + '[getKonyNoSqlConditionFromArray] returning condition is' + conditionString);
            return whereCondition;
        }
        ...
        function getLogicalKonyOperation(logicalOperator) {
            ...
            switch (logicalOperator.toUpperCase()) {
                ...
                default:
                    log.fatal(LOG_PREFIX + "[getLogicalKonyOperation] Can not execute query as logicalOperator can either have values AND or OR");
        ...
        function updateRecordsInTable(txn, tableName, updatedData, condition, fulfill, fail) {
            ...
            kony.nosql.updateRecords(txn, tableName, updatedData, condition)
        ...
        function reconcilePKsInTable(txn, tableName, updatedData, condition, fulfill, fail) {
            ...
            kony.nosql.fetchRecords(txn, tableName, condition)
                .then(resultSet => {
                    ...
                    if (existingDataLength === 0) {
                        ...
                        // delete older data
                        kony.nosql.deleteRecords(txn, tableName, condition)
                            .then(count => {
                                log.trace(LOG_PREFIX, "records to reconcile: " + count);
                                //insert the reconciled records(JSON) in database
                                kony.nosql.addRecords(txn, tableName, existingData)
                                    .then(res => {
        ...
        function insertRecordsInTable(txn, tableName, data, fulfill, fail) {
            ...
            kony.nosql.addRecords(txn, tableName, data)
                .then(function (resultSet) {
        ...
        function insertOrReplaceRecordsInTable(txn, tableName, data, fulfill, fail) {
            ...
            kony.nosql.addOrReplaceRecords(txn, tableName, data)
                .then(function (resultSet) {
        ...
        function fetchRecordsFromTable(txn, tableName, condition, projectionColumns, groupby, groupbyindex, orderby, limit, fulfill, fail) {
            ...
            kony.nosql.fetchRecords(txn, tableName, condition)
                .then(function (resultSet) {
                    ...
                    //group by
                    if (!kony.sdk.isNullOrUndefined(groupby)) {
                        if(!kony.sdk.isNullOrUndefined(groupbyindex)) {
         ...
         */
        KonyNoSQLDatabaseHelper.getDBInstance = function (dbname, nextIndexedDBVersion, dbInfo) {
            log.trace(LOG_PREFIX + "[getDBInstance '" + dbname + "'] called...");
            return new Promise(function (resolve, reject) {

                var dbInfoOfTablesToBeCreated  = {}, listOfTablesToBeDeleted = [];
                if (KonyNoSQLDatabaseHelper.db === null || nosqlDBInstance === null) {
                    ...
                    if (!kony.sdk.isNullOrUndefined(dbInfo)) {
                        ...
                        if (dbInfo.hasOwnProperty(KSDatabaseConstants.KSQueryTypes.CREATE)) {
                            KonyNoSQLDatabaseHelper.db.parseDatabaseSchema(dbInfo[KSDatabaseConstants.KSQueryTypes.CREATE]);
                    ...
                    var upgradeCallback = function (kdb) {
                        for (var tableName in dbInfoOfTablesToBeCreated) {
                            var config = KonyNoSQLDatabaseHelper.db.getTableConfig(dbInfoOfTablesToBeCreated[tableName]);
                            kony.nosql.createTable(kdb, tableName, config)
                        }

                        for (var index in listOfTablesToBeDeleted) {
                            kony.nosql.deleteTable(kdb, listOfTablesToBeDeleted[index]);
                        }
                    };

                    kony.nosql.openDatabase(dbname, nextIndexedDBVersion, upgradeCallback)
                        .then(dbInstance => {

                            //[...] is the syntax for converting DOMStringList into Array
                            KonyNoSQLDatabaseHelper.dbTablesList = Array.from(dbInstance.db[KSDatabaseConstants.OBJECT_STORE_NAMES]);
        ...
        KonyNoSQLDatabaseHelper.prototype.getAllTablesList = function () {
            return new Promise(function (resolve, reject) {
                if (kony.sdk.isNullOrUndefined(nosqlDBInstance)) {
                    ...
                    kony.nosql.getTables(nosqlDBInstance)
                        .then(tables => {
        ...
        KonyNoSQLDatabaseHelper.prototype.closeConnection = function () {
            ...
            return new Promise(function (resolve, reject) {
                kony.nosql.closeDatabase(nosqlDBInstance)
                    .then(res => {
         ...
         * @param {*} limit
         */
        KonyNoSQLDatabaseHelper.prototype.selectDataFromTable = function (tx, tableName, selectWhereClause, projection_col_list, groupBy, groupByIndex, orderBy, limit) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (!kony.sdk.isNullOrUndefined(orderBy)
                    && orderBy.hasOwnProperty("column") && !kony.sdk.isNullOrUndefined(orderBy.column)) {
                ...
                } else if (!kony.sdk.isNullOrUndefined(orderBy)
        ...
        KonyNoSQLDatabaseHelper.getCurrentDBVersion = function (dbName) {
            ...
            if (kony.sdk.isNullOrUndefined(nosqlVersion)) {
                return new Promise(function (resolve, reject) {

                    kony.nosql.databaseVersion(dbName)
                        .then(version => {
                        ...
                        .catch(getVersionError => {
                            ...
                            reject(kony.sdk.OfflineObjects.KSDatabaseSchemaVersion.Version_None);
                        });
                });
            } else {
                return nosqlVersion;
        ...
        KonyNoSQLDatabaseHelper.prototype.isTableFound = function (tablename) {
            return new Promise(function (resolve, reject) {
                if (kony.sdk.isNullOrUndefined(nosqlDBInstance)) {
                    ...
                    kony.nosql.tableExists(nosqlDBInstance, tablename)
                        .then(tablexists => {
        ...
        KonyNoSQLDatabaseHelper.prototype.transaction = function (mode, tablesList, txCallback) {
            ...
            var konyNosqlTransaction = new Promise(function (resolve, reject) {
                kony.nosql.openTransaction(nosqlDBInstance, tablesList, mode, txCallback)
                    .then(konyTransactionResult => {
...
define("KSSDKObject",
    ...
    function (exports, SyncEngine, KSSetupManager, KSBaseORMManager, KSORMManagerFactory, KSSDKObjectRecord,
        ...
        function checkAndSetIfDownloadReconciliationRequired(options) {
            ...
            if (!kony.sdk.isNullOrUndefined(downloadReconciliationRequired)) {
        ...
        KSSDKObject.prototype.createUploadRequest = async function (inputContext, uploadBatchParams) {
            ...
            if (uploadBatchParams.getUploadBatchNumber() === KSInternalConstants.DEFAULT_BATCH_NUMBER && !areThereRecordsToUpload) {
...
define("KSSDKObjectService",
    ...
    function (exports, KSSetupManager, KSSyncMetaInfo, _KSError, KSDeltaContextUtils, _KSDatabaseAPI, KSSyncDatabaseHelper,
        ...
        KSSDKObjectService.prototype.createUploadRequest = async function (inputContext, uploadBatchParams) {
            ...
            if (!areThereRecordsToUpload && uploadBatchParams.getUploadBatchNumber() === KSInternalConstants.DEFAULT_BATCH_NUMBER) {
                logger.info(LOG_PREFIX, "No pending changes found to upload in object");
                uploadRequestParams[KSInternalConstants.ARE_THERE_CHANGES_TO_UPLOAD] = false;
                return uploadRequestParams;
            }
        ...
        function getListOfObjectNameAndFilters() {
            ...
            for (var i = 0; i < lengthOfFullyQualifiedNamesForObjects; i++) {
                ...
                var filter = kony.sdk.isNullOrUndefined(this.filter[KSMetadataUtils.objectNameFromFullyQualifiedName(objectName)]) ? KSInternalConstants.EMPTY_STRING :
    ...
    function (exports, KSCommonUtils, _KSSDKObjectRecord, KSSDKObjectService, _KSDatabaseAPI, KSQueryObjectBuilder, KSSQLQueryGenerator, _KSError, KSOptionsHelper, KSSyncDatabaseHelper, KSMarkForUploadUtils) {
        ...
        KSBaseORMManager.prototype.buildPreparedStatementForGivenCriteria = function (statementBuilder, options) {
            ...
            switch (options[KSCRUDConstants.CRUD_OPTION_CRITERIA]) {
                ...
                case KSCriteria.NONE :
                    ...
                    break;
                default:
                    kony.sdk.logsdk.error(LOG_PREFIX + " : buildPreparedStatementForGivenCriteria",
        ...
        KSBaseORMManager.prototype.setCriteriaInOptions = function (options) {
            if (options.hasOwnProperty(KSPublicConstants.CRUD_OPTION_PRIMARY_KEYS)) {
                ...
                kony.sdk.logsdk.debug(LOG_PREFIX + " : setCriteriaInOptions",
        ...
        KSBaseORMManager.prototype.isReferentialIntegrityValid = async function (sdkObjectRecord, existingRecord) {
            ...
            for (var index = 0; index < relationships.length; index++) {
                ...
                if (areAllNullableForeignKeyValuesAreNull(childObjectAttributes, sdkObjectRecord)) {
                    ...
                    kony.sdk.logsdk.warn(LOG_PREFIX, "All the child object attributes " + columnNames +
        ...
        KSBaseORMManager.prototype.areChangeTrackingOptionsValid = function (options) {
            if (!KSCommonUtils.isNullOrEmptyObject(options)) {
                ...
                if (KSOptionsHelper.isValidBoolTypeOption(options, KSPublicConstants.TRACK_CHANGES)) {
                    kony.sdk.logsdk.info(LOG_PREFIX + " areChangeTrackingOptionsValid: ", "The flag " + KSPublicConstants.TRACK_CHANGES
                        + "is set for the record");
                }

                if (KSOptionsHelper.isValidBoolTypeOption(options, KSPublicConstants.TRACK_INTERMEDIATE_UPDATES)) {
                    kony.sdk.logsdk.info(LOG_PREFIX + " areChangeTrackingOptionsValid: ", "The flag " + KSPublicConstants.TRACK_INTERMEDIATE_UPDATES
                        + "is set for the record");
                }

                if (KSOptionsHelper.isValidBoolTypeOption(options, KSPublicConstants.MARK_FOR_UPLOAD)) {
                    kony.sdk.logsdk.info(LOG_PREFIX + " areChangeTrackingOptionsValid: ", "The flag " + KSPublicConstants.MARK_FOR_UPLOAD
                        + "is set for the record");
                }
            }
            return true;
        ...
        async function performDBOperation(sdkRecord, options) {
            ...
            for (var primaryKey in primaryKeys) {
                if (primaryKeys[primaryKey][KSInternalConstants.ATTRIBUTES_AUTO_GENERATED] &&
                    (trackChanges || kony.sdk.isNullOrUndefined(sdkRecord.objectForKey(primaryKey)))) {
    ...
    function (exports, KSBaseORMManager, KSCommonUtils, KSQueryObjectBuilder, _KSDatabaseAPI, _KSError, KSSQLQueryGenerator, KSOptionsHelper, _KSSDKObjectRecord) {
        ...
        async function buildStatementsForCascadeDeleteOfChildRecords(sdkObject, fkMap, statements, crudOptions) {
            ...
            var queryObj = KSSQLQueryGenerator.buildPreparedStatementsOfTypeRead(sdkObject.name,
                kony.sdk.OfflineObjects.KSTableType.MAIN,
                fkMap);
...
define('KSFlatUploadPayloadBuilder', [
    ...
    function(
        ...
        var KSInternalConstants = kony.sdk.OfflineObjects.KSInternalConstants;
        var KSDatabaseConstants = kony.sdk.OfflineObjects.KSDatabaseConstants;
        var getKSSDKObjectRecordAction = kony.sdk.OfflineObjects.getKSSDKObjectRecordAction;
        ...
        KSFlatUploadPayloadBuilder.prototype.getFlatPayloadToUploadForObject = async function(sdkObject) {
            ...
            if (!KSCommonUtils.isNullOrEmptyObject(operations)) {
                ...
                operationsList.push(kony.sdk.OfflineObjects.KSSDKObjectRecordAction.DO_NOT_TRACK_INTERMEDIATE_UPDATES);
...
define('KSUploadResponseParser',
    ...
    function populateSDKRecordMetadataInRecord(sdkObjectRecord, recordMetadata) {
        ...
        sdkObjectRecord.setOpStatus(!kony.sdk.isNullOrUndefined(opStatus) ? opStatus : sdkObjectRecord.recordMetadata.opStatus);
    }
...
define("KSIncrementalRootMetadataObject",
    ...
    function (exports, KSCommonUtils, KSSQLQueryGenerator, _KSError, KSMetadataUtils) {
        ...
        function populatePropertiesFromIncrementalJSON(jsonDictionary) {
            ...
            if (!kony.sdk.isArray(namespacesArray)) {
            ...
            for (var nameSpaceIndex = 0; nameSpaceIndex < namespacesArray.length; nameSpaceIndex++) {
                ...
                if(kony.sdk.isNullOrUndefined(namespaceDictionary) || !kony.sdk.util.isJsonObject(namespaceDictionary)) {
                    throw new KSError(KSErrorConstants.SETUP_METADATA_NAMESPACES_NIL);
                }
...
define("KSMetadataJSONParser", ["exports", "KSMetadataUtils", "KSCommonUtils", "KSError"],
    function(exports, KSMetadataUtils, KSCommonUtils, _KSError) {
        ...
        function parse (objectServiceContext, metadataResponse){
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Run pass 3 complete for " + objectServiceName);
        ...
        function populateForwardAndReverseDependencyMapUsingOperations(namespace) {
            ...
            for (var objectIndex = 0; objectIndex < objectsArrayLength; objectIndex++) {
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX,
        ...
        function runPass1(objectServiceContext, metadataResponse) {
            ...
            for(var namespaceIndex = 0; namespaceIndex < namespaceLength; namespaceIndex++) {
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "began to add dependencies inside namespace " + namespaceName);
        ...
        function populateObjectMetadataProperties(objectServiceContext, namespace) {
            ...
            for(var objectIndex = 0; objectIndex < objectMetadataLength; objectIndex++) {
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "began to parse object " + objectName + " in Namespace " + namespaceName
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "Parsing completed for object " + objectName + " in Namespace " + namespaceName
                    + " for " + objectServiceName);
            }
        }
        ...
        function populateSoftDeletableAttribute(object) {
            ...
            if(! kony.sdk.util.isNullOrEmptyString(softDeleteAttributeName)) {
                kony.sdk.logsdk.trace(LOG_PREFIX, "began to parse soft delete field in " + object[KSInternalConstants.OBJECTS_NAME]
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "Parsing completed for soft delete field in " + object[KSInternalConstants.OBJECTS_NAME]
                    + " in Namespace " + object[KSInternalConstants.PARENT_NAMESPACE_METADATA][KSInternalConstants.NAMESPACE_NAME]
                    + " for " + object[KSInternalConstants.OBJECT_SERVICE_NAME]);
            }
        }
        ...
        function populatePrimaryKeyAttributes(object) {
            kony.sdk.logsdk.trace(LOG_PREFIX, "began to parse primary keys in " + object[KSInternalConstants.OBJECTS_NAME]
                + " in Namespace " + object[KSInternalConstants.PARENT_NAMESPACE_METADATA][KSInternalConstants.NAMESPACE_NAME]
                + " for " + object[KSInternalConstants.OBJECT_SERVICE_NAME]);
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Parsing completed for primary keys in " + object[KSInternalConstants.OBJECTS_NAME]
                + " in Namespace " + object[KSInternalConstants.PARENT_NAMESPACE_METADATA][KSInternalConstants.NAMESPACE_NAME]
                + " for " + object[KSInternalConstants.OBJECT_SERVICE_NAME]);
        }
        ...
        function populateObjectAttributesProperties(objectServiceName, namespaceName, object) {
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "began to parse attributes in " + objectName
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Parsing completed for attributes in " + objectName
                + " in Namespace " + namespaceName + " for " + objectServiceName);
        }
        ...
        function populateNonNullableFields(object, attribute) {
            if(! attribute[KSInternalConstants.ATTRIBUTES_NULLABLE]) {
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "Parsing completed for non-nullable attribute in " + object[KSInternalConstants.OBJECTS_NAME]
                    + " in Namespace " + object[KSInternalConstants.PARENT_NAMESPACE_METADATA][KSInternalConstants.NAMESPACE_NAME]
                    + " for " + object[KSInternalConstants.OBJECT_SERVICE_NAME]);
            }
        }
                        ...
                        if(objectMetadataDictionary.hasOwnProperty(objectName)) {
                            ...
                            for(var relationshipIndex = 0; relationshipIndex < relationshipLength; relationshipIndex++) {
                                ...
                                if(relationshipType === KSInternalConstants.KSRelationshipTypes.ONE_TO_MANY
                                    ...
                                    kony.sdk.logsdk.trace(LOG_PREFIX, "Populating relationships in object "
        ...
        function deleteExtraKeysInMetadataResponse(metadataResponse){
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Deleted keys " + KSInternalConstants.NAMESPACES + ", "
        ...
        function deleteExtraKeysInNamespace(namespaceMetadata) {
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Deleted keys " + KSInternalConstants.OBJECTS + ", "
        ...
        function deleteExtraKeysInObject(objectMetadata) {
            ...
            kony.sdk.logsdk.trace(LOG_PREFIX, "Deleted keys " + KSInternalConstants.OBJECTS_KEYS + ", "
        ...
        function populateRelationship(metadataResponse, object, relationship) {
            ...
            if(relationshipType === relationship[KSInternalConstants.KSRelationshipTypes.ONE_TO_ONE]) {
                ...
                kony.sdk.logsdk.trace(LOG_PREFIX, "Converted one to one relationship to one to many in object "
...
define("KSObjectServiceMetadataContext", ["exports", "KSSQLQueryGenerator", "KSDatabaseAPI", "KSCommonUtils", "KSError"],
    function(exports, KSSQLQueryGenerator, _KSDatabaseAPI, KSCommonUtils, _KSError) {
        ...
        KSObjectServiceMetadataContext.prototype.doesMetadataTableContainObjectService = function () {
            return !(kony.sdk.isNullOrUndefined(this.deltaContext));
        };
        ...
        KSObjectServiceMetadataContext.prototype.isDeltaContextChanged = function (updatedDeltaContext) {
            if (kony.sdk.isNullOrUndefined(updatedDeltaContext)) {
    ...
    function(exports, KSMetadataUtils, KSNetworkUtils, _KSError, _KSIncrementalRootMetadataObject, KSCommonUtils) {
        ...
        var onNetworkError = function (errorObj) {
            if (this.shouldDoIncrementalSetup) {
                ...
                kony.sdk.logsdk.error(LOG_PREFIX, KSInternalConstants.EM_GENERIC_DATA_FETCHER + " " + this.objectServiceName
                    + ". " + errorObj.message);
                ...
                if (!kony.sdk.isNullOrUndefined(metadataJSON)) {
                    kony.sdk.logsdk.warn(LOG_PREFIX, KSInternalConstants.EM_GENERIC_DATA_FETCHER + " " + this.objectServiceName
                ...
                kony.sdk.logsdk.error(LOG_PREFIX, errorObj.message);
                invokeFailureCallback.call(this, errorObj);
            }
        };
        ...
        function processIncrementalJSONAndUpdateTables(incrementalJSON) {
            try {
                ...
                kony.sdk.logsdk.error(LOG_PREFIX, incrementalRootMetadataException.message);
        ...
        function processJSONAndRootMetadataObject(metadataJSON) {
            ...
            try {
                ...
                kony.sdk.logsdk.error(LOG_PREFIX + metadataParseException.message);
        ...
        function invokeSuccessCallback() {
            ...
            this.successCallback(this.objectServiceName, this.rootMetadataObject, this.tableQueries);
        }
    ...
    function (exports, KSMetadataUtils, KSCommonUtils, KSObjectServiceSetupClass, KSObjectServiceMetadataContextClass,
                ...
                if(failureCount === refreshCounter) {
                    ...
                    try {
                        ...
                        if (failureCount === 0) {
                            ...
                            logger.debug(LOG_PREFIX, "Kony Offline Objects Setup successful.");
                            successCallback(successObj);
                        } else {
                            var clonedSetupStatus = kony.sdk.cloneObject(setupStatus);
                            logger.error(LOG_PREFIX, "Kony Offline Objects Setup failed : "
        ...
        async function createAndUpdateTables() {
            ...
            if(areThereChangesInObjectServiceTableSchema || isDbUpgradeRequired) {
                ...
                if(!kony.sdk.isNullOrUndefined(setupOptions)
                ...
                if(!currentIndexedDBVersion) {
                    ...
                    await createAndUpdateTablesForIncrementalChanges(nextIndexedDBVersion);
                }
            ...
            if(!kony.sdk.isNullOrUndefined(dbUpgradeQueries) || shouldUpgradePropertiesTableVersion) {
        ...
        async function createAndUpdateTablesForFirstTimeUser(nextIndexedDBVersion) {
            ...
            await KSMetadataUtils.createDeleteTablesForAllObjectServices(objectServiceObjectsDDLQueries, nextIndexedDBVersion);
            await KSMetadataUtils.insertIntoMetadataAndMetaInfoTables(objectServiceMetadataContexts);
        }

        async function createAndUpdateTablesForIncrementalChanges(nextIndexedDBVersion) {
            logger.trace(LOG_PREFIX, "Creating and/or updating tables for incremental changes.");
            await KSMetadataUtils.createDeleteTablesForAllObjectServices(objectServiceObjectsDDLQueries, nextIndexedDBVersion);
        ...
        function allObjectServicesSetupFailed() {
            var clonedSetupStatus = kony.sdk.cloneObject(setupStatus);
            logger.error(LOG_PREFIX, "Kony Offline Objects Setup failed : " + JSON.stringify(clonedSetupStatus));
...
define("KSCommonUtils", ["exports", "KSError", "KSRunningTasksManager"], function (exports, _KSError, KSRunningTasksManager) {
    ...
    function isInstanceOfArray(arr) {
        if (kony.sdk.isNullOrUndefined(arr)) {
            return false;
        }
...
define("KSMetadataUtils",
    ...
    ["exports", "KSCommonUtils", "KSSQLQueryGenerator", "KSDatabaseAPI", "KSError", "KSSDKObject"],

    function(exports, KSCommonUtils, KSSQLQueryGenerator, _KSDatabaseAPI, _KSError, _KSSDKObject) {
         ...
         * @throws DatabaseException
         */
        async function createDeleteTablesForAllObjectServices(objectServiceObjectsDDLQueries, nextIndexedDBVersion) {
            var dbName = KSCommonUtils.getOfflineObjectsDatabaseName();
            try {
                logger.trace(LOG_PREFIX, "Entered createDeleteTablesForAllObjectServices...");
                await KSDatabaseAPI.closeDBConnection();
        ...
        async function updatingPropertiesTableDBSchemaVersion(dbInfo, nextIndexedDBVersion) {
            ...
            try {
                ...
                logger.info(LOG_PREFIX, "Completed updating dbSchemaVersion value in konysyncPROPERTIES table.")
            } catch (dbException) {
                logger.error(LOG_PREFIX, "Failed to initialize DB Connection.");
                throw dbException;
            }
        ...
        function doesHierarchyExistInSyncableObject(syncableObject) {
            ...
            if (syncableObject instanceof KSSDKObject !== true) {
                ...
                if (!kony.sdk.isNullOrUndefined(namespaceMetadataMap)) {
        ...
        function getActionFromIncrementalJSONNode(namespaceDictionary) {
            kony.sdk.logsdk.trace(LOG_PREFIX + "[getActionFromIncrementalJSONNode] called...");
        ...
        exports.createDeleteTablesForAllObjectServices = createDeleteTablesForAllObjectServices;
...
define("KSOptionsHelper", ["exports", "KSError", "KSCommonUtils"], function (exports, _KSError, KSCommonUtils) {
    ...
    function validateAndGetQueryParams(options, queryParamsKey){
        ...
        if (!KSCommonUtils.isNullOrEmptyObject(options)
            ...
            && kony.sdk.util.isJsonObject(options[queryParamsKey])) {
...
define("KSSQLQueryGenerator", ["exports", "KSQueryObjectBuilder", "KSCommonUtils"], function (exports, KSQueryObjectBuilder, KSCommonUtils) {
    ...
    /**
     * Method to update DBSchemaVersion with latest version number into konysyncProperties table.
     * @returns {queryBuilderObject}
     */
...
define("OfflineObjectsSPA", ["KSApplicationSync", "SDKObjectSync", "SDKObjectServiceSync"],
    function (KSApplicationSync, _SDKObjectSync, _SDKObjectServiceSync) {
        ...
        kony.sdk.KNYObj.getBinary = function (knyObj, options, fileDownloadStartedCallback, chunkDownloadCompletedCallback,
        ...
        kony.sdk.KNYObjSvc.clearOfflineData = function (knyObjSvc, options, successCallback, failureCallback) {
            kony.sdk.logsdk.trace(" Entering kony.sdk.KNYObjSvc.clearOfflineData");
            var sdkObjectServiceSync = knyObjSvc.getSdkObjectServiceSync();
