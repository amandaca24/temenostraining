...
/* global location */
(function() {
    var $K = kony.$kwebfw$, _navigated = false;

    Object.defineProperty(kony.$kwebfw$.flag, 'navigated', {
        enumerable:false, configurable:true,
        get:function() {return _navigated;}
    });
    Object.defineProperty(kony.$kwebfw$.flag, 'navigated', {
    ...
    $K.defKonyProp($K.ui, [
        {keey:'Form2', value:{}, items:[
            {keey:'onOrientation', value:function(evt) {
                var $K = kony.$kwebfw$, $KD = $K.dom, device = $K.device;
    ...
    var __relayoutResponsiveContainers = function(formModel) {
        ...
        $KW.iterate(formModel, function(widget) {
            ...
            if(widget.layoutType === kony.flex.RESPONSIVE_GRID
            || (widget.parent && widget.parent.layoutType === kony.flex.RESPONSIVE_GRID)) {
    ...
    var _getter = {
        Form2: {
            ...
            contentSizeMeasured: function Form2$_getter_contentSizeMeasured(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, el = null,
    ...
    var _handleAnimation = function Form2$_handleAnimation(cform, cview, pform, pview) {
        var $K = kony.$kwebfw$, $KD = $K.dom,  $KG = $K.globals, timeout = null,
        ...
        if(anim.in) {
            ...
            if(pview && anim.out) {
                $KD.style(pview, {position:'absolute', left:'0px', top:'0px'});
                $KD.style(pview, {
                    animationName: anim.out.effect,
                    animationDuration: anim.out.duration
                });
            }
            $KD.style(cview, {zIndex:2147483646, position:'absolute', left:'0px', top:'0px'});
            $KD.style(cview, {
            ...
            timeout = setTimeout(function() {
                ...
                $KD.style(cview, 'z-index', null);
                $KD.style(cview, {position:null, left:null, top:null});
    ...
    var _handleScrollBar = function Form2$_handleScrollBar() {
        ...
        if($KU.scrollType() === 'native') {
            if(!prop.enableScrolling || prop.scrollDirection === kony.flex.SCROLL_NONE) {
            ...
            if(!prop.enableScrolling || prop.scrollDirection === kony.flex.SCROLL_NONE) {
                ...
                if(prop.horizontalScrollIndicator
                && prop.scrollDirection === kony.flex.SCROLL_VERTICAL) {
                    $KD.style(el.hScroll, 'visibility', 'hidden');
                } else if(prop.verticalScrollIndicator
                && prop.scrollDirection === kony.flex.SCROLL_HORIZONTAL) {
                    $KD.style(el.vScroll, 'visibility', 'hidden');
                }
            }
        }
        ...
        Form2: function Form2$_populateUnderscore() {
            ...
            if(!_.ns) {
                if($KU.is(this.__$kwebfw$ns__, 'string') && this.__$kwebfw$ns__) {
                    ...
                    $KU.defineProperty(_, 'ns', 'kony.ui.Form2', null);
                }
            }
            if(!_.name) {
    ...
    var _removePreviousForm = function Form2$_removePreviousForm() {
        var $K = kony.$kwebfw$, $KD = $K.dom, scroll = this._kwebfw_.ui.scroll;
    ...
    var _renderCurrentForm = function Form2$_renderCurrentForm(holder, view, removePrevForm) {
        ...
        $KD.add(holder, view);
        $KD.style(el.viewport, {top:(el.header.offsetHeight+'px'), bottom:(el.footer.offsetHeight+'px')});
    ...
    var _valid = {
        Form2: {
            ...
            contentOffsetMeasured: function Form2$_valid_contentOffsetMeasured() {
                var $K = kony.$kwebfw$, $KU = $K.utils, flag = false;
            ...
            inTransitionConfig: function Form2$_valid_inTransitionConfig(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, options = null, flag = false;

                if($KU.is(value, 'null')) {
                    flag = true;
                } else if($KU.is(value, 'object')) {
                    if(value.hasOwnProperty('formTransition')) {
                    ...
                    } else if(value.hasOwnProperty('formAnimation')) {
            ...
            scrollDirection: function Form2$_valid_scrollDirection(value) {
                var flag = false, options = [
                    kony.flex.SCROLL_BOTH,
                    kony.flex.SCROLL_HORIZONTAL,
                    kony.flex.SCROLL_NONE,
                    kony.flex.SCROLL_VERTICAL
                ];
    ...
    Object.defineProperty(kony.ui, 'Form2', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.Form2 constructor.
         *
         * @class
         * @namespace   kony.ui
         * @extends     kony.ui.ContainerWidget
            ...
            if(dependentPropertiesValidationMessage) {
                ...
                $KU.each(prop, function(value, key) {
                    ...
                    if(!bconfig.hasOwnProperty(key)) {
                        ...
                        throw new Error('<'+key+'> is available in default widget properties of <kony.ui.Form2>, but not in <_valid.Form2> namespace.');
                    } else {
        ...
        $K.utils.inherits(Form2, kony.ui.ContainerWidget);
         ...
         * @access      protected
         * @method      _render
         * @memberof    kony.ui.Form2
         * @author      Goutam Sahu <goutam.sahu@kony.com>
         *
         * @returns     {HTMLElement} – Form2 view.
         */
        var form2__render = function Form2$_render(tag) {
            var $super = kony.ui.Form2.base.prototype, _ = this._kwebfw_,
                view = _.view, $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
        ...
        var form2_setZoomScale = function Form2$setZoomScale() {
            ...
            if($KW.isFlexContainer(this)) {
                $KU.unsupportedAPI('kony.ui.Form2.setZoomScale');
            } else {
                //Throw Error
            }
        };
