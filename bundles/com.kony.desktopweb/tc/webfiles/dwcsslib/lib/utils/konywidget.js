/* Attribute's meaning
 * -------------------------------------------------------------------------------------------------
 * kcl --------> kony layout type (hflex / vflex / fflex)
 * kfb --------> This is value of kr attribute, which means kony-fake-border
 * ko ---------> kony orientation for widgets like, CheckBoxGroup/RadioButtonGroup/Slider etc.
 * kr ---------> kony role name for our internal plugins/sub role
 * kt ---------> kony theme, for widgets like Calendar/Switch etc.
 * kv ---------> kony widget view type, for widgets like SegmentedUI2 etc.
 * kw ---------> kony widget name
 * kwf --------> kony widget fragment, like calendar popup can render in any context
 * kwg --------> kony widget gesture, can be found in Slider/Images2 etc.
 * kwh --------> kony widget handler
 * kwi --------> kony widget index (e.g. widget inside segment)
 * kwo --------> kony widget owner (widgets inside segment template will have this)
 * kwp --------> kony widget path for automation purpose (e.g. formId_segmentId[0,0]_templateId_buttonId)
 * -------------------------------------------------------------------------------------------------
 */
/* global kony */
/* global constants */
Object.defineProperty(kony.$kwebfw$, 'widget', {configurable:false, enumerable:false, writable:false, value:(function() {
    var _ns = {}, $K = kony.$kwebfw$, _specialARIAkeys = ['lang', 'role'];


    //TODO:: To be verified for each and every widget
    var _a11y = {
            a11yLabel: function(model) {
                var $K = kony.$kwebfw$, $KU = $K.utils, label = model.placeholder,
            ...
            apply: function(model, view, a11y) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, a11yLabel = '';
        ...
        __el = {
            ...
            ListBox: function(model, view, $KU, $KW, $KD, key) {
                var $K = kony.$kwebfw$, $KU = $K.utils, hash = {node:view};
        ...
        _modal = {
            collectAllModals: function(fmodel) {
                var $K = kony.$kwebfw$, $KU = $K.utils, children = null,
                    queue = [], modals = [], info = null, infoList = null;

                if(fmodel.isVisible && fmodel.zIndex !== kony.flex.ZINDEX_AUTO) {
                ...
                while(queue.length) {
                    ...
                    $KU.each(children, function(cmodel, index) {
                        ...
                        if(cmodel.zIndex !== kony.flex.ZINDEX_AUTO
        ...
        _onrender = {
            Label: function() {
                ...
                if(prop.textStyle && prop.textStyle.hasOwnProperty('lineSpacing')) {
                    lineSpacing = prop.textStyle.lineSpacing;

                    //Similar IF block can be found in konylabel.js file Label$_view_textStyle function
    ...
    var _args = {
        ...
        CollectionView: {
            onItemSelect: function(payload) {
                var $K = kony.$kwebfw$, $KU = $K.utils, args = [], item = null,
        ...
        DataGrid: {
            ...
            columnOnClick: function(payload) {
                var $K = kony.$kwebfw$, $KU = $K.utils, args = [];
        ...
        FlexContainer: {
            onBreakpointHandler: function(payload) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KA = $K.app, args = [];
    ...
    var _addToView = function $KW_addToView(view, dom) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, last = null;
    ...
    var _applyGroupA11Y = function $KW_applyGroupA11Y(node, a11y, label, hintId) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, a11yHint = '',
        ...
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
            ...
            prepare = function(widget, who) {
                ...
                for(k in widget._kwebfw_.flex.final) {
                    if(widget._kwebfw_.flex.final.hasOwnProperty(k)) {
                        (function(value, key) {
                            if(key !== 'zIndex') {
                                flex.push((key+'=<'+value+'>'));
                            }
    ...
    var _clearGroupA11y = function $KW_clearGroupA11y(model, masterData, masterDataMap) {
        var $K = kony.$kwebfw$, $KU = $K.utils, uid = model._kwebfw_.uid;
    ...
    var _cloneTemplate = function $KW_cloneTemplate(tpl, data, widgetDataMap, callback) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, clone = null, isCloned = false;

        if($KW.isContainer(tpl)) {
            clone = tpl._konyControllerName ? tpl : $KU.clone(tpl);
            isCloned = (tpl._kwebfw_.is.cloned === true) ? true : false;

            if($KU.is(data, 'object')) {
                $KW.iterate(clone, function(model, pmodel, index) {
                    var $K = kony.$kwebfw$, $KU = $K.utils, defaultProp = null, dataId = '',
                        wap = null, _ = model._kwebfw_, prop = undefined,
                        userwidget = (_.uwi instanceof kony.ui.UserWidget) ? _.uwi : null;
                            ...
                            if(['metaInfo'].indexOf(key) < 0) {
                                ...
                                if(userwidget) {
                                    ...
                                    && $KU.hasEventProperty(model, key)) {
                                        controller = _kony.mvc.getComponentController(container._konyControllerName, container.appName);
                                        ...
                                        if(!(model instanceof kony.ui.GroupWidget && ['selectedKeyValue', 'selectedKeyValues'].indexOf(key) !== -1)) {
                                            model[key] = value;
                                        }
                                    }
                                }
    ...
    var _cmodel = function $KW_cmodel(model, childIdOrIndex) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, id = '';
        ...
        if(model) {
            ...
            if(rmodel) {
                ...
                if(_.uwi instanceof kony.ui.UserWidget) {
                    component = _.uwi;
                } else if(rmodel instanceof kony.ui.UserWidget) {
                    component = rmodel;
                }
            }
        }
    ...
    var _details = function $KW_details(omodel, index, model, view) {
        var $K = kony.$kwebfw$, $KU = $K.utils, i = 0,
    ...
    var _disabled = function $KW_disabled(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, omodel = null, disabled = false;
    ...
    var _dismissPickers = function $KW_dismissPickers(fmodel) {
        ...
        $KU.each(fmodel._kwebfw_.pickers, function(uid) {
        ...
        delete fmodel._kwebfw_.pickers;
    };
    ...
    var _fire = function $KW_fire(model, type, scope, payload, callbacks) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
            $KA = $K.app, $KAR = $K.automation.recorder, item = null,
            omodel = $KW.omodel(model), args = [model], context = null,
            cf = $KW.model($KA.currentFormUID),
            noForceLayoutEventList = ['onInit', 'preShow'];
    ...
    var _fmodel = function $KW_fmodel(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, hmodel = _hmodel(model);
    ...
    var _focusableElement = function $KW_focusableElement(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
            dom = null, el = null;
        ...
        if(model) {
            ...
            if(el.node) {
                if($KU.is(_a11y.view[_name(model)], 'function')) {
                    dom = _a11y.view[_name(model)](el.node);
                } else {
                    dom = el.node;
                }
            }
    ...
    var _getContentOffsetValues = function $KW_getContentOffsetValues(model, offset) {
        var $K = kony.$kwebfw$, $KU = $K.utils, x = 0, y = 0;

        if($KU.is(offset, 'object')) {
            if($KU.is(offset.x, 'number')) {
                offset.x += model._kwebfw_.layoutUnit || kony.flex.DEFAULT_UNIT;
            }
            if($KU.is(offset.y, 'number')) {
                offset.y += model._kwebfw_.layoutUnit || kony.flex.DEFAULT_UNIT;
            }
        }
    ...
    var _getDefaultHeight = function $KW_getDefaultHeight(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, omodel = null,
        ...
        if(model) {
            ...
            if(_isTextDrivenWidget(model) || name ===  'Image2') {
                ...
                if($KW.inPercent(prop.height) && _.is.template
                   && omodel && omodel.autogrowMode === kony.flex.AUTOGROW_HEIGHT) {
                    height = '100dp';
                } else {
                    height = '220dp';
                }
    ...
    var _getGroupSelectedKeyValueByKey = function $KW_getGroupSelectedKeyValueByKey(model, key) {
        var $K = kony.$kwebfw$, $KU = $K.utils, keyValue = null,
    ...
    var _getNonConstructorProperties = function $KW_getNonConstructorProperties(widgetName) {
        ...
        if(properties.hasOwnProperty(widgetName)) {
    ...
    var _getTemplate = function $KW_getTemplate(omodel, tpl) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KA = $K.app,
            _ = $KU.is(omodel, 'widget') ? omodel._kwebfw_ : null, tplId = null;

        if(tpl) {
            tplId = ($KU.is(tpl, 'string')) ? tpl : tpl.id;

            if($KU.is(tpl, 'string')) {
                $KA.allowSetter_konyControllerName = true;
                tpl = _kony.mvc.initializeSubViewController(tpl);
                delete $KA.allowSetter_konyControllerName;
    ...
    var _getWidgetDataMapPath = function $KW_getWidgetDataMapPath(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, tmodel = null, path = [];
    ...
    var _handleOnScrollWidgetPosition = function $KW_handleOnScrollWidgetPosition() {
        var $K = kony.$kwebfw$, $KD = $K.dom, $KU = $K.utils, $KW = $K.widget, onScrollWidgets = [],
    ...
    var _hmodel = function $KW_hmodel(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, omodel = null, hmodel = null;
    ...
    var _index = function $KW_index(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, proxy = null,
    ...
    var _inheritedProperties = function $KW_inheritedProperties(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, prop = {};

        model = $KW.model(model);

        if($KU.is(model, 'widget', 'inherited')) {
            $KU.each(Object.getOwnPropertyNames(model), function(value) {
                if(value !== '_kwebfw_' && !this._kwebfw_.prop.hasOwnProperty(value)) {
    ...
    var _inModalContainer = function $KW_inModalContainer(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, omodel = null, flag = false;
    ...
    var _inPercent = function $KW_inPercent(value) {
        var $K = kony.$kwebfw$, $KU = $K.utils, flag = false, len = 0;
    ...
    var _inTemplate = function $KW_inTemplate(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, rmodel = null, flag = false;
    ...
    var _isAriaRoleAllowed = function $KW_isAriaRoleAllowed(model, role) {
        var $K = kony.$kwebfw$, $KW = $K.widget, a11y = null, name = '', flag = false;
        ...
        if(model) {
            ...
            if(name === 'Button' && ['link'].indexOf(role) >= 0) {
                ...
                flag = true;
            } else if(name === 'SegmentedUI2' && ['table', 'grid'].indexOf(role) >= 0) {
    ...
    var _isContainer = function $KW_isContainer(model) {
        ...
        if(model && model instanceof kony.ui.ContainerWidget) {
            flag = true;
        }
    ...
    var _isFixedHeight = function $KW_isFixedHeight(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, final = null, flag = false;
    ...
    var _isFlexContainer = function $KW_isFlexContainer(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, flag = false,
            prop = null, layoutList = [
                kony.flex.FREE_FORM, kony.flex.FLOW_HORIZONTAL,
                kony.flex.FLOW_VERTICAL, kony.flex.RESPONSIVE_GRID
            ];
        ...
        if(model) {
            ...
            if(model instanceof kony.ui.FlexContainer
            || (model instanceof kony.ui.Form2
    ...
    var _isResponsiveContainer = function $KW_isResponsiveContainer(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KF = kony.flex, flag = false, prop = null;
    ...
    var _isScrollableWidget = function $KW_isScrollableWidget(model, scrollDirection) {
        ...
        if($KU.is(model, 'widget', 'FlexScrollContainer') || $KU.is(model, 'widget', 'Form2')) {
            ...
            if(prop.scrollDirection != kony.flex.SCROLL_NONE && !_disabled(model)) {
                flag = true;
            }
        ...
        && ($KU.is(model, 'widget', 'FlexScrollContainer') || $KU.is(model, 'widget', 'Form2'))) {
            if(prop.scrollDirection === kony.flex.SCROLL_VERTICAL
            && prop.scrollDirection === scrollDirection) {
                flag = true;
            } else if(prop.scrollDirection === kony.flex.SCROLL_HORIZONTAL
            && prop.scrollDirection === scrollDirection) {
                flag = true;
            } else if(prop.scrollDirection === kony.flex.SCROLL_BOTH
            && prop.scrollDirection === scrollDirection) {
                flag = true;
            } else {
                flag = false;
    ...
    var _iterate = function $KW_iterate(model, callback, config) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, _ = null, output = null;
    ...
    var _layout = function $KW_layout(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, layout = '',
    ...
    var _markRelayout = function $KW_markRelayout(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, $KA = $K.app,
            cf = $KW.model($KA.currentFormUID), relayout = null;
    ...
    var _modifySelectedIndexes = function $KW_modifySelectedIndexes(selectedIndexes, depth, position, count) {
        var $K = kony.$kwebfw$, $KU = $K.utils, indexesToBeRemoved = [];
        ...
        && $KU.is(selectedIndexes, 'array')) {
            $KU.each(selectedIndexes, function(value, index) {
                var $K = kony.$kwebfw$, $KU = $K.utils, vlen = -1;
    ...
    var _onPropertyChange = function $KW_onPropertyChange(model, key, old) {
        var $K = kony.$kwebfw$, $KU = $K.utils, owner = _omodel(model);
    ...
    var _onRender = function $KW_onRender(nodes) {
        ...
        $KU.each(nodes, function(view) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
                $KD = $K.dom, model = $KW.model(view), name = _name(model);

            _onrender[name] && _onrender[name].call(model);

            $KU.each($KD.find(view, '[kw="Label"],[kw="ListBox"],[kw="CustomWidget"],[kw="FlexScrollContainer"],[kw="SegmentedUI2"]'), function(node) {
                var $K = kony.$kwebfw$, $KW = $K.widget,
    ...
    var _pmodel = function $KW_pmodel(model, arg1) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, _ = null,
    ...
    var _proxy = function $KW_proxy(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, proxy = $KW.model(model);

        if(proxy && proxy instanceof kony.ui.UserWidget) {
            proxy = proxy._kwebfw_.proxy;

            if(!(proxy instanceof kony.ui.FlexContainer
            || proxy instanceof kony.ui.FlexScrollContainer)) {
                proxy = null;
            }
        }
            ...
            if(widget) {
                ...
                if(scroll.x !== scrollX || scroll.y !== scrollY) {
                    ...
                    if(scroll.status === 'ended') {
                        ...
                        scroll.maxX = (scroll.width - this.offsetWidth);
                        scroll.maxY = (scroll.height - this.offsetHeight);
    ...
    var _removeAllSkinsFromUI = function $KW_removeAllSkinsFromUI(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, prop = null, el = null;
    ...
    var _retainScrollPosition = function $KW_retainScrollPosition(model, retainScrollPosition) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, el = $KW.el(model),
    ...
    var _returnPosition = function $KW_returnPosition(position, dimension) {
        var $K = kony.$kwebfw$, $KU = $K.utils, result = 0,
            patt1 = /\d+%/g, patt2 = /\d+px/g, operator = '',
    ...
    var _rmodel = function $KW_rmodel(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, rmodel = null;
    ...
    var _scrollBy = function $KW_scrollBy(model, x, y, duration) {
        var $K = kony.$kwebfw$, $KW = $K.widget, scroll = null;
    ...
    var _scrollTo = function $KW_scrollTo(model, x, y, duration, algo) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, el = null,
    ...
    var _scrollToSetFocus = function $KW_scrollToSetFocus(model, positionObj, animate, time) {
        var $K = kony.$kwebfw$, $KU =$K.utils, scrollableView = null;
    ...
    var _scrollElementToParentScroller = function $KW_scrollElementToParentScroller(model, parentModel) {
        ...
        if(elView.node && parentView.viewport) {
            ...
            if(_isScrollableWidget(parentModel, kony.flex.SCROLL_VERTICAL)
            || _isScrollableWidget(parentModel, kony.flex.SCROLL_BOTH)) {
            ...
            if(_isScrollableWidget(parentModel, kony.flex.SCROLL_HORIZONTAL)
            || _isScrollableWidget(parentModel, kony.flex.SCROLL_BOTH)) {
    ...
    var _setContentOffset = function $KW_setContentOffset(model, offset, animate) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
            prop = model._kwebfw_.prop, el = null, widgets = null, value = {};
            ...
            if(arguments.length === 2 && !$KU.is(animate, 'boolean')) {
                ...
                if($KU.is(value.x, 'integer') && $KU.is(value.y, 'integer')) {
                    ...
                    if($KU.scrollType() !== 'native') {
                        ...
                        el = $KW.el(model);
                        el.viewport.scrollLeft = model._kwebfw_.ui.scroll.x = value.x;
                        el.viewport.scrollTop  = model._kwebfw_.ui.scroll.y = value.y;
                    }
                } else {
    ...
    var _shouldApplyRTL = function $KW_shouldApplyRTL(model, property) {
        var $K = kony.$kwebfw$, $KA = $K.app , currentlocale = kony.i18n && $KA.currentLocale,
    ...
    var _shouldLazyLoad = {
        SegmentedUI2: function() {
            var $K = kony.$kwebfw$, $KW = $K.widget, flag = false, prop = this._kwebfw_.prop;

            if($KW.isFixedHeight(this)
            && this._kwebfw_.prop.viewType === constants.SEGUI_VIEW_TYPE_TABLEVIEW
            && (kony.$kwebfw$.behavior.enableLazyLoadForSegment || prop.enableLazyLoad)) {
                flag = true;
            }
    ...
    var _skinHandlers = function $KU_skinhandlers() {
        return {
            ...
            background: function(skinObj) {
                var $K = kony.$kwebfw$, $KD = $K.dom, $KW = $K.widget, value;
            ...
            fonts: function(skinObj) {
                var $K = kony.$kwebfw$, $KD = $K.dom, $KU = $K.utils, $KW = $K.widget, fontObj,
    ...
    var _skinUtils = {
        background: function(config) {
            ...
            if(bgType === kony.skin.BACKGROUND_TYPE_SINGLE_COLOR && config.backgroundColor) {
                background = $KW.skinUtils.processColorValue(config.backgroundColor);
            } else if(bgType === kony.skin.BACKGROUND_TYPE_IMAGE && config.backgroundImage) {
                background = 'url("' + $KU.getImageURL(config.backgroundImage) + '")';
            } else if(bgType === kony.skin.BACKGROUND_TYPE_MULTI_STEP_GRADIENT) {
                background  = $KW.skinUtils.msGradient(config);
            } else if(bgType === kony.skin.BACKGROUND_TYPE_TWO_STEP_GRADIENT) {
        ...
        border: function(config) {
            ...
            if(config.borderType === kony.skin.BORDER_TYPE_SINGLE_COLOR && config.borderColor){
        ...
        font: function(config) {
            ...
            if($KU.is(config.fontStyle, 'string')) {
                if(config.fontStyle === kony.skin.FONT_STYLE_UNDERLINE) {
        ...
        getBaseFontSize: function() {
            ...
            if(stylesheet) {
                ...
                $KU.each(rules, function(rule) {
                    if(rule.selectorText === '.konycustomcss') {
        ...
        msGradient: function(config) {
            var $K = kony.$kwebfw$, $KU = $K.utils, i = 0, $KW = $K.widget,
                msColor = config.backgroundColorMultiStepGradient, gradient;

            if(msColor) {
                gradient = 'to ' + ['top', 'right', 'bottom', 'left'][[
                                kony.skin.MULTI_STEP_GRADIENT_TYPE_TO_BOTTOM,
                                kony.skin.MULTI_STEP_GRADIENT_TYPE_TO_LEFT,
                                kony.skin.MULTI_STEP_GRADIENT_TYPE_TO_TOP,
                                kony.skin.MULTI_STEP_GRADIENT_TYPE_TO_RIGHT
        ...
        prepareTextShadowStyle: function (config) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, style = '',
                shadowColor = config.textShadowColor || '#000000',
        ...
        twoStepGradient: function(config) {
            var $K = kony.$kwebfw$, $KU = $K.utils, bottomcolor = '', gradient = null, topcolor = '',
                tsColor = config.backgroundColorTwoStepGradient, $KW = $K.widget, percentage = null;

            if(tsColor) {
                percentage = (tsColor.style === kony.skin.TWO_STEP_GRADIENT_STYLE_VERTICAL_SPLIT
                || tsColor.style === kony.skin.TWO_STEP_GRADIENT_STYLE_HORIZONTAL_SPLIT) ? '50%' : '';
                ...
                gradient = 'to ' + ['bottom', 'bottom', 'right', 'right'][[
                                kony.skin.TWO_STEP_GRADIENT_STYLE_VERTICAL_GRADIENT,
                                kony.skin.TWO_STEP_GRADIENT_STYLE_VERTICAL_SPLIT,
                                kony.skin.TWO_STEP_GRADIENT_STYLE_HORIZONTAL_GRADIENT,
                                kony.skin.TWO_STEP_GRADIENT_STYLE_HORIZONTAL_SPLIT
    ...
    var _removeSensitiveText = function $KW_removeSensitiveText(model) {
        ...
        $KW.iterate(model, function(widget) {
            ...
            if($KU.is(widget, 'widget', 'SegmentedUI2')
            || $KU.is(widget, 'widget', 'CollectionView')) {
                var $K = kony.$kwebfw$, $KU = $K.utils, templates = null;
    ...
    var _tmodel = function $KW_tmodel(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, tmodel = null;
    ...
    var _toggleChildren = function $KW_toggleChildren(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom,
            ...
            shouldBeDisabled = ['BUTTON', 'FIELDSET', 'INPUT', 'OPTGROUP', 'OPTION', 'OBJECT', 'SELECT', 'TEXTAREA', 'A', 'AREA', 'IFRAME', 'DETAILS', 'AUDIO', 'VIDEO', '[tabindex]', '[kony-bkp-tabindex]'],
            listOfElements = dom.querySelectorAll(shouldBeDisabled);

        $KU.each(listOfElements, function(item) {
            var $K = kony.$kwebfw$, $KD = $K.dom, disabled = model._kwebfw_.disabled;

            if(disabled) {
                if($KD.hasAttr(item, 'tabindex')) {
                    $KD.setAttr(item, 'kony-bkp-tabindex', $KD.getAttr(item, 'tabindex'));
                ...
                if(['BUTTON', 'FIELDSET', 'INPUT', 'OPTGROUP', 'OPTION', 'SELECT', 'TEXTAREA'].indexOf(item.tagName) >= 0) {
                    if($KD.hasAttr(item, 'disabled')) {
                        $KD.setAttr(item, 'kony-bkp-disabled', 'disabled');
                    } else {
                        $KD.setAttr(item, 'kony-dev-disabled', 'disabled');
                    }
                    $KD.setAttr(item, 'disabled', 'disabled');
                } else if(['A', 'AREA', 'IFRAME','DETAILS'].indexOf(item.tagName) >= 0) {
                    $KD.setAttr(item, 'tabindex', -1);
                    $KD.setAttr(item, 'kony-dev-tabindex', -1);
                }
            } else {
                if(['BUTTON', 'FIELDSET', 'INPUT', 'OPTGROUP', 'OPTION', 'SELECT', 'TEXTAREA'].indexOf(item.tagName) >= 0) {
                    if($KD.hasAttr(item, 'kony-dev-disabled')) {
                        $KD.removeAttr(item, 'disabled');
                        $KD.removeAttr(item, 'kony-dev-disabled');
                    } else if($KD.hasAttr(item, 'kony-bkp-disabled')) {
                        $KD.setAttr(item, 'disabled', $KD.getAttr(item, 'kony-bkp-disabled'));
                        $KD.removeAttr(item, 'kony-bkp-disabled');
                    }
                } else if(['A', 'AREA', 'IFRAME', 'DETAILS'].indexOf(item.tagName) >= 0) {
                    if($KD.hasAttr(item, 'kony-dev-tabindex')) {
                        $KD.removeAttr(item, 'tabindex');
                        $KD.removeAttr(item, 'kony-dev-tabindex');
                    }
                }

                if($KD.hasAttr(item, 'kony-bkp-tabindex')) {
                    $KD.setAttr(item, 'tabindex', $KD.getAttr(item, 'kony-bkp-tabindex'));
                    $KD.removeAttr(item, 'kony-bkp-tabindex');
                }
            }
        });

        if(disabled) {
            $KD.addCls(dom, '-kony-blocker');
        } else {
            $KD.removeCls(dom, '-kony-blocker');
        }
    };


    var _tpmodel = function $KW_tpmodel(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget, _ = null, tpmodel = null;
    ...
    var _updateScroll = function $KW_updateScroll(model) {
        ...
        //The same IF BLOCK is there in "konywidget.js" file "_scrollTo" function
    ...
    var _normalizeGroupMasterData = function $KW_normalizeGroupMasterData(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, data = model._kwebfw_.prop.masterData;

        if(data) {
            $KU.each(data, function(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, keey =  value[0];
    ...
    var _normalizeGroupMasterDataMap = function $KW_normalizeGroupMasterDataMap(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, data = model._kwebfw_.prop.masterDataMap;

        if(data) {
            $KU.each(data[0], function(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, keey =  value[data[1]];
