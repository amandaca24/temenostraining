...
(function () {
    ...
    var getParentInfo = function() {
        var result = {};
        result.appName = _kony.automation.params.parentApp;
        result.windowRef = window.opener;
        if(_kony.automation.params) {
            result.queryParams = _kony.automation.params;
        } else {
            result.queryParams = null;
        }
        _kony.automation.parentApp = {"appName" : result.AppName, "windowRef" : result.windowRef};
        return result;
    }

    var launchApp = function(config) {
        var $K = kony.$kwebfw$, $KU = $K.utils,appInfo = null, appURL = "";

        $KU.log({api:'kony.automation.launchApp', enter:true});
        if(config === null || config === undefined || JSON.stringify(config) === JSON.stringify({}) || !config.appName) {
            $KAUtils.throwExceptionInsufficientArguments();
        }

        if(!_kony.automation.IntegrationTests[config.appName]) {
            $KAUtils.throwExceptionInvalidAppName();
        }

        appInfo = _kony.automation.IntegrationTests[config.appName];
        ...
        $KU.log({api:'kony.automation.launchApp', exit:true});
    ...
    var _waitForEvent = function(options) {
        var $K = kony.$kwebfw$, $KU = $K.utils,maxWaitTime = null;

        $KU.log({api:'kony.automation.waitForEvent', enter:true});
        if(options.maxWait && typeof options.maxWait === 'number') {
            maxWaitTime = options.maxWait;
        }

        _kony.automation.eventDetails.pauseTest = true;
        _kony.automation.eventDetails.eventName = options.eventName;

        $KU.log({api:'kony.automation.waitForEvent', exit:true});
        return new Promise(function(resolve, reject) {
    ...
    var _waitForResume = function(timeOut, resolve, reject) {
        ...
        setTimeout(function() {
            if(_kony.automation.eventDetails.pauseTest){
                if(typeof timeOut !== 'undefined' && timeOut !== null  && timeOut === 0) {
                ...
                if(_kony.automation.eventDetails.data) {
                    resolve({"eventReceived" : true, "data" : _kony.automation.eventDetails.data});
                } else {
    ...
    var _sendEvent = function(targetDetails) {
        var $K = kony.$kwebfw$, $KU = $K.utils, targetWindow = null, appInfo = null;

        $KU.log({api:'kony.automation.sendEvent', enter:true});
        if(targetDetails.windowRef) {
            targetWindow = targetDetails.windowRef;
        } else {
            return false;
        }
        if(!_kony.automation.IntegrationTests[targetDetails.appName] || !targetDetails.eventName) {
            return false;
        }

        appInfo = _kony.automation.IntegrationTests[targetDetails.appName];

        targetWindow.postMessage({"mode":"CAT", "event" : targetDetails.eventName, "data" : targetDetails.data}, appInfo.URL);
        $KU.log({api:'kony.automation.sendEvent', exit:true});
        return true;
    }

    var _sendEventToParent = function(targetDetails) {
        var $K = kony.$kwebfw$, $KU = $K.utils, targetWindow = null, parentAppURL = "", parentApp = _kony.automation.parentApp;

        $KU.log({api:'kony.automation.sendEventToParent', enter:true});
        ...
        parentAppURL = _kony.automation.IntegrationTests[parentApp.appName]["URL"];

        targetWindow.postMessage({"mode":"CAT", "event" : targetDetails.eventName, "data" : targetDetails.data}, parentAppURL);
        $KU.log({api:'kony.automation.sendEventToParent', exit:true});
        return true;
    }
    ...
    var _getWidgetsByFilter = function(widgetPath, filters, searchableWidgets) {
        ...
        $K = kony.$kwebfw$, $KD = $K.dom, $KW = $K.widget, $KA = $K.app, $KU = $K.utils;
    ...
    var _waitForLoadingScreenToDismiss = function(timeout) {
        ...
        $KU.log({api:'kony.automation.waitForLoadingScreenToDismiss', enter:true});
        if(timeout && typeof timeout !== 'number') {
            $KAUtils.throwExceptionInvalidArgumentType();
        }

        $KU.log({api:'kony.automation.waitForLoadingScreenToDismiss', exit:true});
        return new Promise(function(resolve, reject) {
    ...
    var _connectToHost = function() {
        var karConfig = _kony.automation;
    ...
    var _waitForConnection = function(timeOut, resolve, reject) {
        var waitTime, karConfig = _kony.automation;
    ...
    var _onOpenListener = function(event) {
        var karConfig = _kony.automation.server, wSocket = karConfig.wSocket;
    ...
    var _messageListner = function(messageEvent) {
        var karConfig = _kony.automation.server,message = JSON.parse(messageEvent.data);
    ...
    var _sendMessage = function(options) {
        ...
        if(_kony.automation.server) {
            karConfig = _kony.automation.server;
    ...
    var _waitForMessage = function(options) {
        var $K = kony.$kwebfw$, $KU = $K.utils,maxWaitTime = null, karConfig = _kony.automation;

        $KU.log({api:'kony.automation.waitForMessage', enter:true});
        if(options.maxWait && typeof options.maxWait === 'number') {
            maxWaitTime = options.maxWait;
        }
        if(!karConfig.server) {
        ...
        $KU.log({api:'kony.automation.waitForMessage', exit:true});
        return new Promise(function(resolve, reject) {
