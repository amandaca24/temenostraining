...
(function (ns) {
    ...
    ns.renderApplicationSkins = function(skinsObj, platform, formFactor, cgArgs) {
        try {
            var skinBuffer = new konyUtil.StringBuffer();
            var retinaskinBuffer = new konyUtil.StringBuffer();
            var basecssMap = {};
            ...
            if(cgArgs.theme_constants) {
                result = ns.processSkinningConstants(cgArgs, skinsNameSorted, skinsObj, basefont);
                skinBuffer.append(result);
            }

            //process custom fonts if any.
            result = ns.processCustomFonts(cgArgs, skinsNameSorted, skinsObj, themeName, cgArgs.processThemeContants);
            skinBuffer.append(result);
            ...
            for(var snsIdx = 0; snsIdx < skinsNameSorted.length; snsIdx++) {
                ...
                */
                ...
                if(konyUtil.isplatformspa(platform)) {
                ...
                if(wType == "progress indicator") {
                    ...
                    //Currently this option is missing in the visualizer.
                    ...
                    result = konyUtil.templaterenderer(templateFile, skinMap);
                    skinBuffer.append(result);
                    ...
                    if(wType == "Switch") {
                        ...
                        cssUtils.processSwitchFontProperties(appSkin, skinMap, platform, cgArgs.processThemeContants);
                        cssUtils.processBorderProperties(appSkin, skinMap, platform);
                    } else {
                        cssUtils.processBackGroundProperties(appSkin, skinMap, platform);
                        cssUtils.processFontProperties(appSkin, skinMap, platform, cgArgs.processThemeContants);
                        cssUtils.processBorderProperties(appSkin, skinMap, platform);
                    }
                    ...
                    var result = konyUtil.templaterenderer(templateFile, skinMap);

                    if(result.includes("var(")) {
                        result = result.replace(/#var\(/g,"var(");
                        result = processSkinPropertyforIE(result, cgArgs.processThemeContants);
                    }
    ...
    function populateSkinName(appSkin, skinName) {
        ...
        var widgetTag = widgetTagMap[wType];
        if(!widgetTag) {
            widgetTag = widgetTagMap['default'];
        }
        if(wType === 'CheckBoxGroup' || wType === 'RadioButtonGroup') {
            skinName += ", "+ widgetTag + "." + skinName + "-hover, "
                        + widgetTag + "[kw=" + wType + "] >div." + skinName + "-focus";
        } else if(wType === 'ListBox') {
            skinName += ", "+ widgetTag + "." + skinName + "-hover, "
                        + widgetTag + "[kw]." + skinName + "-focus, "
                        + "div."+ skinName + "-hover, div[kw]."+skinName +"-focus, "
                        + "div[kw='ListBox'][kv='editableview'] ~ ul>li." +skinName +"-hover";
        } else if(wType === 'RichText') {
            skinName += ", "+ widgetTag + "." + skinName + "-hover, "
                        + widgetTag + "[kw]." + skinName + "-focus, "
                        + "div[kw=" + wType + "]."+ skinName +"-link a ,"
                        + "div[kw=" + wType + "]."+ skinName +"-linkfocus a:active";
        } else if(wType === 'TabPane') {
            skinName += ", " + widgetTag + "[kw= '" + wType + "'][kv = 'collapsibleview'] div[tabid]."+ skinName + "-focus, "
                        + widgetTag + "[kw= '" + wType + "'][kv = 'tabview'] ul[kr='tabs']>li[tabid]." +skinName + "-focus";
        } else if(wType === 'Label') {
            skinName += ", [kw]." + skinName + "-hover, "
                         + "label[kw]." + skinName + "-focus, "
                         + "h1[kw]." + skinName + "-focus, "
                         + "h2[kw]." + skinName + "-focus, "
                         + "h3[kw]." + skinName + "-focus, "
                         + "h4[kw]." + skinName + "-focus, "
                         + "h5[kw]." + skinName + "-focus, "
                         + "h6[kw]." + skinName + "-focus, "
                         + "div[kw]." + skinName + "-focus, "
                         + "span[kw]." + skinName + "-focus, "
                         + "p[kw]." + skinName + "-focus ";
         } else {
             skinName += ", "+ widgetTag + "." + skinName + "-hover, "
                        + widgetTag + "[kw]." + skinName + "-focus";
    ...
    function generateBaseCSSforWidgetSkins(skinsObj, basecssMap, platform) {
        var resultBuffer = new konyUtil.StringBuffer();
        ...
        var id = "";
        var baseSkins = [];
        var skinIds = new konyUtil.StringBuffer();
        ...
        if(length) skinIds.append('.konyfw_baseskincss,');
        ...
        var result = konyUtil.templaterenderer(templateFile, basecssMap);
        return result;
    }
    ...
    function setProgressSkinProperties(skinMap, appSkin) {
        ...
        if(pgcolor && pgcolor.trim().length > 0) {
            pgcolor = konyUtil.processColorOnly(pgcolor);
    ...
    ns.processSkinningConstants = function(cgArgs, skinsList, skinsObj, basefont) {
        var isNewLib = cgArgs.newlib, result = '', processThemeContants = {};

        var processConstantsToRoot = function(themeConstants, basefont) {
            var conStr = ':root { \n', colors = null, fonts = null;

            colors = themeConstants.color;
            fonts = {
                'fontStyle':themeConstants.fontStyle,
            ...
            if(colors && Object.keys(colors).length > 0) {
                for(var ckey in colors) {
                    var pVal = konyUtil.processColorOnly(colors[ckey]);
                    conStr += '--'+ckey+' : \ '+ pVal +'; \n';
                    processThemeContants[ckey] = pVal;
                }
            }

            //Fonts processing to result
            if(fonts && Object.keys(fonts).length > 0) {
                for(var type in fonts) {
                    if(type === 'fontSize') {
                        for(var fkey in fonts[type]) {
                            var pVal = deduceFontSizeValue(fonts[type][fkey], basefont);
                            conStr += '--'+fkey+' : \ '+ pVal +'; \n';
                            processThemeContants[fkey] = pVal;
                        }
                    } else {
                        for(var fkey in fonts[type]) {
                            conStr += '--'+fkey+' : \ '+fonts[type][fkey]+'; \n';
                            processThemeContants[fkey] = fonts[type][fkey];
                        }
                    }
                }
            }
            ...
            cgArgs.processThemeContants = processThemeContants;

            return conStr;
        };

        var skinVarProcess = function(skin) {
            for(var skinprop in skin) {
                var sknVal = skin[skinprop];
                if(typeof sknVal === 'string' && sknVal.includes('$')) {
                    skin[skinprop] = 'var(--'+ sknVal.replace('$', '')+')';
                } else if(sknVal instanceof Array) {
                    for(var i = 0; i < sknVal.length; i++) {
                        if(typeof sknVal[i] === 'string' && sknVal[i].includes('$')) {
        ...
        var skinVarToValueProcess = function(skin, flatThemeConstants) {
            for(var skinprop in skin) {
                var sknVal = skin[skinprop];
                if(typeof sknVal === 'string' && sknVal.includes('$')) {
                    skin[skinprop] = flatThemeConstants[sknVal.replace('$', '')];
                } else if(sknVal instanceof Array) {
                    for(var i = 0; i < sknVal.length; i++) {
                        if(typeof sknVal[i] === 'string' && sknVal[i].includes('$')) {
                            skin[skinprop][i] = flatThemeConstants[sknVal[i].replace('$', '')];
                        }
                    }
                } else if(typeof sknVal === 'object') {
                    skinVarToValueProcess(sknVal, flatThemeConstants);
                }
            }
        };

        var deduceFontSizeValue = function(size, basefont) {
            var propVal = '', fontSize = parseInt(size),
            suggestedFontSize = (fontSize * basefont) /100;

            suggestedFontSize = (Math.round(suggestedFontSize * 10)/10).toFixed(1);
            propVal += suggestedFontSize + 'px';
        ...
        try {
            if(isNewLib) {
                cgArgs.processThemeContants = null;
                result = processConstantsToRoot(cgArgs.theme_constants, basefont);
                ...
                for(var type in cgArgs.theme_constants) {
                    Object.assign(flatThemeConstants, cgArgs.theme_constants[type]);
                }
                for(var i = 0; i < skinsList.length; i++) {
                    skin = skinsObj[skinsList[i]];
    ...
    ns.processCustomFonts = function(cgArgs, skinsList, skinsObj, themeName, themeConstants) {
        ...
        try {
            if(customfonts && customfonts.length > 0) {
                ...
                for(var i = 0; i < skinsList.length; i++) {
                    skin = skinsObj[skinsList[i]];
                    fontName = skin["font_name"];
                    if(themeConstants && typeof fontName === 'string'
                    && fontName.includes('var(--')) {
                        fontConst = fontName.match(/var\(--([^\s]*)\)/)[1];
                        fontName = themeConstants[fontConst];
                    }
                ...
                customFontsInUse = konyUtil.getUniqueValuesArray(customFontsInUse); //removing duplicates
                map["customfontsinuse"] = customFontsInUse;
                var templateFile = velocitytemplatesmap.spa.customfonts;
                var result = konyUtil.templaterenderer(templateFile, map);
            }
            return result;
        } catch(e) {
        ...
        if(fs.existsSync(fontsDir)) {
            ...
            if(platformfontsdirname) {
                ...
                if(fs.existsSync(platformfontsdir)) {
                    konyUtil.collectItemsRecursivesly(platformfontsdir, customFontsList);
