...
/* global constants */
(function() {
    var $K = kony.$kwebfw$, $KW = $K.widget, _map = {}, _root = {},
    ...
    //Here widget can be an instanceof kony.ui.BasicWidget or kony.ui.UseWidget
    ...
    var _addRemovedWidget = function UI$_addRemovedWidget(widget) {
        ...
        $KW.iterate(proxy, function(model) {
            var $K = kony.$kwebfw$, $KU = $K.utils, _ = model._kwebfw_;

            if(_.uwi instanceof kony.ui.UserWidget) {
    ...
    var _addWidget = function UI$_addWidget(widget, fmodel, rmodel, init, append, index) {
        ...
        if(this.layoutType === kony.flex.RESPONSIVE_GRID) {
    ...
    ****************************************************************************/
    //Here widgets can hold both widget, which is an instanceof kony.ui.BasicWidget/UseWidget
    //This function must be called in the scope of widget instance
    ...
    var _addWidgetsToView = function UI$_addWidgetsToView(widgets, index) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KD = $K.dom,
            fragment = null, holder = null, fmodel = $KW.fmodel(this), clen = 0,
            rendered = (fmodel && $KW.isRendered(fmodel)), fchildren = null;
            ...
            if(widgets.length > 0 && $KU.is(holder, 'dom')) {
                if(widgets.length > 1) {
                    ...
                    $KU.each(widgets, function(widget) {
                        if(widget.isVisible || $K.F.RIVW) {
                            if(widget instanceof kony.ui.UserWidget) {
                    ...
                    if(widgets[0] instanceof kony.ui.UserWidget) {
                ...
                if(fragment) {
                    ...
                    if($KU.is(index, 'number')) {
                        clen = $KD.children(holder).length;

                        if(index >= 0 && index < clen) {
                            $KD.addAt(holder, fragment, index);
                        } else if(index >= clen) {
    ...
    var _applyFlexCSS = {
        fflex: {
            ...
            horizontal: function UI$_applyFlexCSS_fflex_horizontal(cmodel, pmodel, cview, pview, forced, rendered) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KD = $K.dom, final = null;
        ...
        rflex: {
            ...
            widthDefined: function UI$_applyFlexCSS_rflex_widthDefined(final, cmodel, pmodel, cview, pview) {
                var $K = kony.$kwebfw$, $KD = $K.dom, calcMarginLeft = '', calcMarginRight = '',
        ...
        fflex: {
            heightDefined: {
                ...
                withTopBottom: function UI$_applyFrame_fflex_heightDefined_withTopBottom(cmodel, pmodel, cview, pview) {
                    var $K = kony.$kwebfw$, $KD = $K.dom, final = cmodel._kwebfw_.flex.final;
            ...
            heightNotDefined: {
                ...
                withTopCenterY: function UI$_applyFrame_fflex_heightNotDefined_withTopCenterY(cmodel, pmodel, cview, pview) {
                    var $K = kony.$kwebfw$, $KD = $K.dom, calc = '',
        ...
        hflex: {
            heightDefined: {
                ...
                withOnlyCenterY: function UI$_applyFrame_hflex_heightDefined_withOnlyCenterY(cmodel, pmodel, cview, pview) {
                    var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom, calc = '',
                        final = cmodel._kwebfw_.flex.final, property = 'marginTop';
    ...
    var _canMeasureHeight = function UI$_canMeasureHeight(model, context) {
        ...
        if(model.autogrowMode === kony.flex.AUTOGROW_HEIGHT) {
            flag = true;
            ...
            if($KW.inPercent(prop.height) && _.is.template
               ...
               && omodel.autogrowMode === kony.flex.AUTOGROW_HEIGHT) {
                flag = false;
            } else {
                flag = true;
            }
        ...
        if(flag && model.autogrowMode === kony.flex.AUTOGROW_HEIGHT
    ...
    var _cleanFlexCSS = function UI$_cleanFlexCSS(cmodel, cview, pview) {
        var $K = kony.$kwebfw$, $KU = $K.utils,
            $KD = $K.dom, prop = cmodel._kwebfw_.prop;
        ...
        if(cview) {
            ...
            if(cmodel instanceof kony.ui.ContainerWidget
    ...
    var _convertFlexPropertyToCssUnit = function UI$_convertFlexPropertyToCssUnit(value, layoutUnit) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KF = kony.flex, unit = '', len = 0;
    ...
    //Though model can be an instanceof kony.ui.BasicWidget or kony.ui.UseWidget
    //But, ultimately it always operates on widget, which is an instance of kony.ui.BasicWidget
    //This function must be called in the scope of widget instance
    //And this widget instance must be a FORM/TAB/TEMPLATE/COMPONENT widget.
    //Here this can never be an instanceof kony.ui.UserWidget
    var _createWidgetHierarchy = function UI$_createWidgetHierarchy(model) {
        ...
        $KW.iterate(model, function(widget) {
            ...
            if(this !== parent && $KU.is(this[widget.id], 'widget')) {
    ...
    var _deduceFinalFlexPropeties = {
        common: function UI$_deduceFinalFlexPropeties_common(model, prop, context, flex, forced, rendered) {
            ...
            if(!$KU.is(model, 'widget', 'Form2')) {
                ...
                if(model._kwebfw_.is.template) {
                    ...
                    && omodel.layout !== kony.collectionview.LAYOUT_CUSTOM) {
                        if($K.ui.CollectionView.isHeaderOrFooter.call(omodel, model)) {
                            ...
                            if(omodel.layout === kony.collectionview.LAYOUT_VERTICAL) {
                                flex.height = '100%';
                            } else {
                                flex.width = '100%';
                            }
        ...
        fflexHeightDefined: function UI$_deduceFinalFlexPropeties_fflexHeightDefined(model, prop, context, flex) {
            var $K = kony.$kwebfw$, $KW = $K.widget, layoutUnit = model._kwebfw_.layoutUnit;
        ...
        rflexWidthDefined: function UI$_deduceFinalFlexPropeties_rflexWidthDefined(model, prop, context, flex) {
            ...
            var $KU = $K.utils, $KA = $K.app, curBrkPoint = $KA.currentBreakpoint,
                brkPoints = null, curBrkPointIndex = null, i, cf = $KW.model($KA.currentFormUID),
    ...
    var _flagForMeasure = function UI$_flagForMeasure(uid, position, direction, cview, pview, reverse) {
        var $K = kony.$kwebfw$, $KU = $K.utils, measure = null;
    ...
    var _flagForMutate = function UI$_flagForMutate(cmodel, cview) {
        var $K = kony.$kwebfw$, $KU = $K.utils,
            $KW = $K.widget, clayout = $KW.layout(cmodel);
    ...
    var _getter = {
        ...
        ContainerWidget: {
            reverseLayoutDirection: function ContainerWidget$_getter_reverseLayoutDirection(value) {
                var $K = kony.$kwebfw$, $KW = $K.widget, prop = this._kwebfw_.prop;

                if(prop.layoutType === kony.flex.FLOW_HORIZONTAL
    ...
    var _ifContainsMediaWidgetThenPlay = function $KW_ifContainsMediaWidgetThenPlay(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget;
    ...
    var _ifContainsSegmentHandleLazyLoad = function $KW_ifContainsSegmentHandleLazyLoad(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget;
    ...
    var _invokeAddWidgets = function UI$_invokeAddWidgets(arg0) {
        var $K = kony.$kwebfw$, $KU = $K.utils, self = null;
    ...
    var _populateUnderscore = {
        BasicWidget: function BasicWidget$_populateUnderscore() {
            ...
            if(!_.ns) $KU.defineProperty(_, 'ns', 'kony.ui.BasicWidget', null);
            ...
            if(!_.skinStateObj) $KU.defineProperty(_, 'skinStateObj', {bgType:kony.skin.BACKGROUND_TYPE_SINGLE_COLOR}, null);
        ...
        ContainerWidget: function ContainerWidget$_populateUnderscore() {
            ...
            //NOTE:: Any changes to _ (underscore) may need a change in
            //       _cleanUnderscore function of konyui.js file.
            if(!_.ns) $KU.defineProperty(_, 'ns', 'kony.ui.ContainerWidget', null);
        ...
        GroupWidget: function GroupWidget$_populateUnderscore() {
            ...
            //NOTE:: Any changes to _ (underscore) may need a change in
            //       _cleanUnderscore function of konyui.js file.
            if(!_.ns) $KU.defineProperty(_, 'ns', 'kony.ui.GroupWidget', null);
        ...
        UserWidget: function UserWidget$_populateUnderscore() {
            ...
            //NOTE:: Any changes to _ (underscore) may need a change in
            //       _cleanUnderscore function of konyui.js file.
            if(!_.ns) $KU.defineProperty(_, 'ns', 'kony.ui.UserWidget', null);
    ...
    var _postInitialization = {
        BasicWidget: function BasicWidget$_postInitialization(bconfig, lconfig, pspconfig) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, _ = this._kwebfw_,
        ...
        ContainerWidget: function ContainerWidget$_postInitialization(bconfig, lconfig, pspconfig) {
            var $K = kony.$kwebfw$, $KU = $K.utils, _ = this._kwebfw_, self = this;
    ...
    var _playMediaWidget = function $KW_playMediaWidget(model) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
            el = $KW.el(model);
    ...
    var _relayout = function UI$_relayout(model, forced, callback) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, playout = '',
        ...
        if(_.view && (model.isVisible || $K.F.RIVW)) {
            ...
            && _dirty.templates[_.uid]) {
                ...
                if(pmodel) {
                    ...
                    playout = $KW.layout(kony.flex.FLOW_VERTICAL);
                }
        ...
        if($KW.isOwnerWidget(model)) {
            ...
            relayoutTemplate = function(tpl, cb) {
                ...
                $KW.iterate(tpl, function(widget) {
                    var $K = kony.$kwebfw$, _ = widget._kwebfw_;
    ...
    var _relayoutPassiveTriggerer = {
        BasicWidget: function BasicWidget$_relayoutPassiveTriggerer() {
            return ['focusSkin', 'hoverSkin', 'skin'];
        },
    ...
    //Here cmodel can never be an instanceof kony.ui.UserWidget
    //This function must be called in the scope of widget instance
    var _removeWidgetHierarchy = function UI$_removeWidgetHierarchy(cmodel, rmodel, remove) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, _ = cmodel._kwebfw_;
    ...
    var _resolveMeasures = function UI$_resolveMeasures() {
        ...
        $KU.each(_measures, function $each_resolveMeasures_height_width_pheight(info, uid) {
            var $K = kony.$kwebfw$, $KU = $K.utils, pelement = null;
        ...
        $KU.each(_measures, function $each_resolveMeasures_position(info, uid) {
            var $K = kony.$kwebfw$, $KU = $K.utils, calc = '',
                $KW = $K.widget, $KD = $K.dom, sprop = '',
                cmodel = null, pmodel = null, playout = '';
    ...
    var _resolveMutates = function UI$_resolveMutates() {
        var $K = kony.$kwebfw$, $KU = $K.utils, cacheOverflowY = {};
        ...
        if($KU.scrollType() === 'native') {
            $KU.each(_mutates, function $each_resolveMutates_getScrollPosition(view) {
                var $K = kony.$kwebfw$, scroll = null, el = null,
        ...
        $KU.each(_mutates, function $each_resolveMutates_setHeightToZero(view) {
            ...
            if(view) {
                ...
                $KU.each($KW.children(model), function $each_resolveMutates_cacheOverflowY(cmodel) {
                    var $K = kony.$kwebfw$, $KD = $K.dom, _ = cmodel._kwebfw_, overflow = '';
        ...
        $KU.each(_mutates, function $each_resolveMutates_setScrollPosition(view) {
            var $K = kony.$kwebfw$, $KU = $K.utils, scroll = null,
    ...
    var _setCenterYStyleInsideVerticalLayout = function UI$_setCenterYStyleInsideVerticalLayout(value, fprop, cmodel, cview, pview) {
        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, flag = true,
    ...
    var _setter = {
        BasicWidget: {
            ...
            isVisible: function BasicWidget$_setter_isVisible(old) {
                ...
                if(old === false) {
                    $KW.iterate(this, function(model) {
                        var $K = kony.$kwebfw$, $KW = $K.widget, _ = model._kwebfw_;
            ...
            left: function BasicWidget$_setter_left(old) {
                var $K = kony.$kwebfw$, $KW = $K.widget,
                    ...
                    left = prop.left, right = prop.right;
        ...
        ContainerWidget: {
            ...
            gutterX: function ContainerWidget$_setter_gutterX() {
                ...
                if(prop.layoutType === kony.flex.RESPONSIVE_GRID) {
                    $KU.each($KW.children(this), function(cmodel) {
                        $KW.markRelayout(cmodel);
                    }, this);
                }
    ...
    var _shouldDeduceModalContainer = {
        ...
        onZindexChange: function UI$_shouldDeduceModalContainer_onZindexChange(newVal, oldVal) {
            ...
            if(modal) {
                if($KU.is(newVal, 'integer') && oldVal === kony.flex.ZINDEX_AUTO) {
                    flag = true;
                } else if($KU.is(oldVal, 'integer') && newVal === kony.flex.ZINDEX_AUTO) {
    ...
    //Here model can never be an instance of kony.ui.UserWidget
    //This function must be called in the scope of widget instance
    var _updateComponentWidgetPath = function UI$_updateComponentWidgetPath(model) {
        var $K = kony.$kwebfw$, $KW = $K.widget;

        $KW.iterate(model, function(widget) {
            var $K = kony.$kwebfw$, $KU = $K.utils, _ = widget._kwebfw_;
    ...
    var _valid = {
        BasicWidget: {
            accessibilityConfig: function BasicWidget$_valid_accessibilityConfig(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils,
                    ...
                    name = '', supportedTags = {
                        ...
                        SegmentedUI2: ['ul', 'ol', 'div']
                    };

                if($KU.is(value, 'object') || $KU.is(value, 'null')) {
                    flag = true;

                    if(value && value.hasOwnProperty('tagName')) {
                        tag = value.tagName;
                        name = $KW.name(this);
            ...
            autogrowMode: function BasicWidget$_valid_autogrowMode(value) {
                ...
                if(value === kony.flex.AUTOGROW_HEIGHT
                || value === kony.flex.AUTOGROW_NONE) {
                    flag = true;
                }
            ...
            fontStyle: function BasicWidget$_valid_fontStyle(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, flag = false, options = [
                    '',
                    kony.skin.FONT_STYLE_ITALIC,
                    kony.skin.FONT_STYLE_NONE,
                    kony.skin.FONT_STYLE_UNDERLINE
                ];
            ...
            fontWeight: function BasicWidget$_valid_fontWeight(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, flag = false, options = [
                    '',
                    kony.skin.FONT_WEIGHT_BOLD,
                    kony.skin.FONT_WEIGHT_NORMAL
                ];
            ...
            frame: function BasicWidget$_valid_frame(value) {
                var flag = false, $K = kony.$kwebfw$, $KU = $K.utils;
            ...
            id: function BasicWidget$_valid_id(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, regexp = null, flag = false;
            ...
            paddingInPixel: function BasicWidget$_valid_paddingInPixel(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, flag = true;
            ...
            parent: function BasicWidget$_valid_parent(value) {
                var flag = false, $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget;
            ...
            transform: function BasicWidget$_valid_transform(value) {
                ...
                if($KU.is(value, 'null')
                || value instanceof kony.$kwebfw$.Transform) {
                    flag = true;
                }
            ...
            zIndex: function BasicWidget$_valid_zIndex(value) {
                ...
                if(value === kony.flex.ZINDEX_AUTO
        ...
        ContainerWidget: {
            _konyControllerName: function ContainerWidget$_valid__konyControllerName(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, flag = false;

                if($KU.is(value, 'string') && value
                && (!this._konyControllerName
                || value === this._konyControllerName)) {
                    if($KU.is(this, 'widget', 'Form2')) {
                        flag = true;
                    } else if($KU.is(this, 'widget', 'component')) {
                        flag = true;
                    } else if($KU.is(this, 'widget', 'FlexContainer')) {
                        if($K.app.allowSetter_konyControllerName
            ...
            layoutType: function ContainerWidget$_valid_layoutType(value) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KA = $K.app,
                    $KF = kony.flex, flag = false, children = null, options = [
                        $KF.FREE_FORM,
                        $KF.FLOW_HORIZONTAL,
                        $KF.FLOW_VERTICAL,
                        $KF.VBOX_LAYOUT,
                ...
                if(value === kony.flex.RESPONSIVE_GRID
    ...
    var _view = {
        BasicWidget: {
            ...
            backgroundColor: function BasicWidget$_view_backgroundColor(el, old) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KD = $K.dom, backgroundStyle;

                this._kwebfw_.skinStateObj.bgType = kony.skin.BACKGROUND_TYPE_SINGLE_COLOR;
            ...
            backgroundColorMultiStepGradient: function BasicWidget$_view_backgroundColorMultiStepGradient(el, old) {
                ...
                this._kwebfw_.skinStateObj.bgType = kony.skin.BACKGROUND_TYPE_MULTI_STEP_GRADIENT;
            ...
            backgroundColorTwoStepGradient: function BasicWidget$_view_backgroundColorTwoStepGradient(el, old) {
                ...
                this._kwebfw_.skinStateObj.bgType = kony.skin.BACKGROUND_TYPE_TWO_STEP_GRADIENT;
            ...
            backgroundImage: function BasicWidget$_view_backgroundImage(el, old) {
                ...
                this._kwebfw_.skinStateObj.bgType = kony.skin.BACKGROUND_TYPE_IMAGE;
            ...
            blur: function BasicWidget$_view_blur(el, old) {
                var $K = kony.$kwebfw$, $KD = $K.dom, filter = '';
            ...
            contentAlignment: function BasicWidget$_view_contentAlignment(el, old) {
                ...
                || $KU.is(this, 'widget', 'TextArea2')
                || $KU.is(this, 'widget', 'TextBox2')) {
                    $KD.removeCls(el.node, ('-kony-ca-'+old));
                    $KD.addCls(el.node, ('-kony-ca-'+this.contentAlignment));
                } else if($KU.is(this, 'widget', 'Calendar')) {
                    $KD.removeCls(el.input, ('-kony-ca-'+old));
                    $KD.addCls(el.input, ('-kony-ca-'+this.contentAlignment));
                }
            },

            cornerRadius: function BasicWidget$_view_cornerRadius(el, old) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, vlaue;
            ...
            fontStyle: function BasicWidget$_view_fontStyle(el, old) {
                var $K = kony.$kwebfw$, $KD = $K.dom, $KW = $K.widget, fontStyle = this.fontStyle,
                    widgets = ['FlexScrollContainer', 'FlexContainer', 'Form2', 'Map', 'Video', 'Browser'],
                    textStyle = this.textStyle ? this.textStyle.strikeThrough : null;

                if(widgets.indexOf($KW.name(this)) !== -1
                || ($KW.name(this) === 'Label' && textStyle
                && fontStyle === kony.skin.FONT_STYLE_UNDERLINE)) {
                    return;
                }

                if(old === kony.skin.FONT_STYLE_UNDERLINE) {
                    $KD.style(el.node, 'text-decoration', null);//todo
                }

                if(this.fontStyle) {
                    if(fontStyle === kony.skin.FONT_STYLE_UNDERLINE) {//TODO: label has to verify
            ...
            isVisible: function BasicWidget$_view_isVisible(el, old) {
                var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, $KD = $K.dom,
                    $KA = $K.app, $KG = $K.globals, _ = this._kwebfw_, pmodel = null,
                    fmodel = $KW.fmodel(this), widgets = null, view = el.node, index = -1,
                    owner = null, next = null, holder = null, modal = null;
                ...
                if(view || this.isVisible) {
                    ...
                    } else { //Form2 or top level template widget of 'SegmentedUI2', 'CollectionView', 'DataGrid' or 'MenuContainer'
                        if(!this.isVisible) {
            ...
            shadowColor: function BasicWidget$_view_shadowColor(el, old) {
                var $K = kony.$kwebfw$, $KD = $K.dom, $KW = $K.widget, shadowStyle = '';
            ...
            zIndex: function BasicWidget$_view_zIndex(el, old) {
                var $K = kony.$kwebfw$, $KW = $K.widget, $KD = $K.dom,
                    $KA = $K.app, fmodel = $KW.fmodel(this), modal = null;
    ...
    $K.defKonyProp($KW, [
        ...
        {keey:'root', value:function UI$_$KW_root(view, arg1, arg2) {
            var $K = kony.$kwebfw$, $KW = $K.widget, _ = null,
            ...
            if(model) {
                ...
                && !_.rid && !_.pid && _.is.hasOwnProperty(arg1)
    ...
    *                                                                          *
    ****************************************************************************/
    Object.defineProperty(kony.ui, 'BasicWidget', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.BasicWidget constructor.
         *
         * @class
         * @namespace   kony.ui
        ...
        var BasicWidget = function BasicWidget(bconfig, lconfig, pspconfig) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, self = null,
            ...
            if(this === kony.ui) {
                ...
                if(['kony.ui.BasicWidget', 'kony.ui.ContainerWidget', 'kony.ui.GroupWidget'].indexOf(this._kwebfw_.ns) >= 0) {
                    delete this._;
                    ...
                    if(dependentPropertiesValidationMessage) {
                        ...
                        if(!bconfig.isPreValidated) {
                            ...
                            $KU.each(prop, function(value, key) {
                                ...
                                if(!bconfig.hasOwnProperty(key)) {
                                    ...
                                    throw new Error('<'+key+'> is available in default widget properties of <kony.ui.BasicWidget>, but not in <_valid.BasicWidget> namespace.');
                                } else {
                        ...
                        $KU.each(_view.BasicWidget, function(value, key) {
                            ...
                            $KU.defineSetter(self, key, function BasicWidget$_setter(val) {
                                var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, rmodel = null,
         ...
         * @access      protected
         * @method      _flush
         * @memberof    kony.ui.BasicWidget
         * @author      Goutam Sahu <goutam.sahu@kony.com>
         *
         * @returns     void
         */
        var basic__flush = function BasicWidget$_flush(config) {
            var $K = kony.$kwebfw$, $KD = $K.dom, _ = this._kwebfw_, $KU = $K.utils,
                prop = _.prop, konyControllerName = prop._konyControllerName,
            ...
            if(konyControllerName && !config.fromDestoryCallstack) {
                _kony.mvc.destroyController(konyControllerName);
            }
        };


        /**
         * Builds the view layer for kony.ui.BasicWidget.
         *
         * @access      protected
         * @method      _render
         * @memberof    kony.ui.BasicWidget
        ...
        var basic__render = function BasicWidget$_render(tag, children) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KD = $K.dom, $KW = $K.widget, el = null,
                $KF = kony.flex, omodel = null, parent = $KW.pmodel(this), _ = this._kwebfw_,
        ...
        var basic_addGestureRecognizer = function BasicWidget$addGestureRecognizer(type, options, callback) {
            var $K = kony.$kwebfw$, $KU = $K.utils, gesture = null, identifier = '';
        ...
        var basic_removeGestureRecognizer = function BasicWidget$removeGestureRecognizer(identifier) {
            var $K = kony.$kwebfw$, $KU = $K.utils, position = -1, gtype = '',
        ...
        var basic_setGestureRecognizer = function BasicWidget$setGestureRecognizer(type, options, callback) {
            var $K = kony.$kwebfw$, $KU = $K.utils, identifier = '';
        ...
        $K.defKonyProp(BasicWidget.prototype, [
    ...
    *                                                                          *
    *                                                                          *
    *                         kony.ui.ContainerWidget                          *
    *                                                                          *
    *                                                                          *
    ****************************************************************************/
    Object.defineProperty(kony.ui, 'ContainerWidget', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.ContainerWidget constructor.
         *
         * @class
         * @namespace   kony.ui
            ...
            if(dependentPropertiesValidationMessage) {
                ...
                $KU.each(prop, function(value, key) {
                    ...
                    if(!bconfig.hasOwnProperty(key)) {
                        ...
                        throw new Error('<'+key+'> is available in default widget properties of <kony.ui.ContainerWidget>, but not in <_valid.ContainerWidget> namespace.');
                    } else {
                ...
                $KU.each(_view.ContainerWidget, function(value, key) {
                    ...
                    $KU.defineSetter(self, key, function ContainerWidget$_setter(val) {
                        var $K = kony.$kwebfw$, $KU = $K.utils, proxy = this,
        ...
        $K.utils.inherits(ContainerWidget, kony.ui.BasicWidget);
         ...
         * @access      protected
         * @method      _flush
         * @memberof    kony.ui.ContainerWidget
         * @author      Goutam Sahu <goutam.sahu@kony.com>
         *
        ...
        var container__flush = function ContainerWidget$_flush(config) {
            var $super = kony.ui.ContainerWidget.base.prototype, _ = this._kwebfw_,
                $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, id = _.prop.id;
            ...
            if($KU.is(this, 'widget', 'Form2')) {
                delete _root[id];
            } else if(_.uwi instanceof kony.ui.UserWidget) {
        ...
        var container__render = function ContainerWidget$_render(tag, children) {
            var $super = kony.ui.ContainerWidget.base.prototype,
                el = null, view = this._kwebfw_.view, $K = kony.$kwebfw$,
        ...
        var container_addAt = function ContainerWidget$addAt(widget, index) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, children = null;
        ...
        var container_executeOnParent = function Container$executeOnParent(callback) {
            var $K = kony.$kwebfw$, $KW = $K.widget, pmodel = null, args = [];

            if(this._konyControllerName) {
                pmodel = $KW.closest($KW.model(this._kwebfw_.owner), function(widget) {
                    return (widget._konyControllerName) ? true : null;
                ...
                if(pmodel) {
                    ...
                    _kony.mvc.executeInJsContext(pmodel, callback, args);
                }
            }
        };
        ...
        var container_forceLayout = function ContainerWidget$forceLayout(arg0) {
            ...
            if(!$KW.isFlexContainer(this)) {
                ...
                if($KW.isRendered(fmodel)) {
                    if($KU.is(arg0, 'boolean')) {
                        forced = arg0;
                    } else if(!$KU.is(this, 'widget', 'Form2')
                    && this._kwebfw_.forceLayout === true) {
                        //e.g. forceLayout of kony.ui.Map callout template
                        forced = true;
                        isForm = false;
                    ...
                    if(forced || fmodel._kwebfw_.relayout) {
                        ...
                        if(isForm) {
                            ...
                            //       for widgets of kony.ui.Map callout template
        ...
        var container_removeAt = function ContainerWidget$removeAt(index) {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget, widget = null;
        ...
        var container_scrollToBeginning = function Container$scrollToBeginning() {
            var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
                _ = this._kwebfw_, scroll = _.ui.scroll, el = null;

            if(this instanceof kony.ui.Form2
            || this instanceof kony.ui.FlexScrollContainer) {
                scroll.x = scroll.y = 0;
                el = $KW.el(this);

                if(el.viewport) {
                    if($KU.scrollType() === 'native') {
                        el.viewport.scrollLeft = scroll.x;
                        el.viewport.scrollTop = scroll.y;
                    }
                }
            } else {
        ...
        var container_setDefaultUnit = function ContainerWidget$setDefaultUnit(unit) {
            var $K = kony.$kwebfw$, $KW = $K.widget, $KF = kony.flex;
        ...
        $K.defKonyProp(ContainerWidget.prototype, [
    ...
    *                                                                          *
    ****************************************************************************/
    Object.defineProperty(kony.ui, 'GroupWidget', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.GroupWidget constructor.
         *
         * @class
         * @namespace   kony.ui
         * @extends     kony.ui.BasicWidget
            ...
            if(dependentPropertiesValidationMessage) {
                ...
                if(!bconfig.isPreValidated) {
                    ...
                    $KU.each(prop, function (value, key) {
                        ...
                        if(!bconfig.hasOwnProperty(key)) {
                            ...
                            throw new Error('<' + key + '> is available in default widget properties of <kony.ui.GroupWidget>, but not in <_valid.GroupWidget> namespace.');
                        } else {
                ...
                $KU.each(_view.GroupWidget, function(value, key) {
                    ...
                    $KU.defineSetter(self, key, function GroupWidget$_setter(val) {
                        var $K = kony.$kwebfw$, $KU = $K.utils, rmodel = null, valid = false,
        ...
        $K.utils.inherits(GroupWidget, kony.ui.BasicWidget);
         ...
         * @access      protected
         * @method      _flush
         * @memberof    kony.ui.GroupWidget
         * @author      Goutam Sahu <goutam.sahu@kony.com>
         *
         * @returns     void
         */
        var group__flush = function GroupWidget$_flush() {
            var $super = kony.ui.GroupWidget.base.prototype,
        ...
        /**
         * Builds the view layer for kony.ui.GroupWidget.
         *
         * @override
         * @access      protected
         * @method      _render
        ...
        var group__render = function GroupWidget$_render(tag, children) {
            var $super = kony.ui.GroupWidget.base.prototype,
                view = this._kwebfw_.view, $K = kony.$kwebfw$, $KU = $K.utils;
        ...
        $K.defKonyProp(GroupWidget.prototype, [
    ...
    *                                                                          *
    ****************************************************************************/
    Object.defineProperty(kony.ui, 'UserWidget', {configurable:false, enumerable:false, writable:false, value:(function() {
        var $K = kony.$kwebfw$;


        /**
         * kony.ui.UserWidget constructor.
         *
         * @class
         * @namespace   kony.ui
         * @author      Goutam Sahu <goutam.sahu@kony.com>
            ...
            if(dependentPropertiesValidationMessage) {
                ...
                if(!bconfig.isPreValidated) {
                    ...
                    $KU.each(prop, function (value, key) {
                        ...
                        if(!bconfig.hasOwnProperty(key)) {
                            ...
                            throw new Error('<' + key + '> is available in default widget properties of <kony.ui.UserWidget>, but not in <_valid.UserWidget> namespace.');
                        } else {
                ...
                $KU.each(_view.UserWidget, function(value, key) {
                    ...
                    $KU.defineSetter(self, key, function UserWidget$_setter(val) {
                        var $K = kony.$kwebfw$, $KU = $K.utils, $KW = $K.widget,
                            valid = false, message = '', rmodel = null, old = null;
        ...
        $K.defKonyProp(UserWidget.prototype, [
    ...
    *                                                                          *
    ****************************************************************************/
    Object.defineProperty(kony.ui, 'Alert', {configurable:false, enumerable:false, writable:false, value:(function() {
        var Alert = function Alert(config, arg1, arg2) {
            var $K = kony.$kwebfw$, $KU = $K.utils, ret = false,
    ...
    Object.defineProperty(kony.ui, 'createAnimation', {configurable:false, enumerable:false, writable:false, value:(function() {
        var createAnimation = function createAnimation(animDef) {
            var $K = kony.$kwebfw$, $KU = $K.utils, animator = null;

            $KU.log({api:'kony.ui.createAnimation', enter:true});
            animator = new $K.Animator(animDef);
            $KU.log({api:'kony.ui.createAnimation', exit:true});
    ...
    Object.defineProperty(kony.ui, 'defineExtendedWidget', {configurable:false, enumerable:false, writable:false, value:function(child, parent, protoFunctionDictionary) {
        var $K = kony.$kwebfw$, $KU = $K.utils, ns = child, name = '', pkg = window;

        if(!(parent === kony.ui.Button || parent === kony.ui.Image2
        || parent === kony.ui.Label || parent === kony.ui.Switch
        || parent === kony.ui.TextArea2 || parent === kony.ui.TextBox2)) {
    ...
    Object.defineProperty(kony.ui, 'makeAffineTransform', {configurable:false, enumerable:false, writable:false, value:(function() {
        var makeAffineTransform = function makeAffineTransform() {
            var $K = kony.$kwebfw$, $KU = $K.utils, transform = null;

            $KU.log({api:'kony.ui.makeAffineTransform', enter:true});
            transform = new $K.Transform();
            $KU.log({api:'kony.ui.makeAffineTransform', exit:true});
