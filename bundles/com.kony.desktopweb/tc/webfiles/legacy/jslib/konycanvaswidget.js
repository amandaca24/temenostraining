...
$KW.Canvas = (function() {
        ...
        if(dataArr instanceof Array) {
            ...
            for(i = 0; i <= len; ++i) {
                if(!dataArr[i].shapeid) {
                    throw new KonyError(6001, "Invalid shapesData", "shapesData does not have mandatory value shapeid.");
                } else if(!dataArr[i].shapeType) {
                    throw new KonyError(6001, "Invalid shapesData", "shapesData does not have mandatory value shapeType.");

                } else if(validDataObj[dataArr[i].shapeid]) {
                    throw new KonyError(6002, "Invalid shapesData", "shapesData has duplicate shape identifier. " + dataArr[i].shapeid);
                } else {
                    if(kony.canvas.SHAPE_TYPE_LINE === dataArr[i].shapeType) {
                        validateLineData(dataArr[i]);
                    } else {
                        throw new KonyError(6001, "Invalid shapesData", "Not a valid shapeType.");
                    }
    ...
    var validateLinePts = function(points, flag) {
        ...
        if(points instanceof Array) {
            if(points.length != 2 ) {
                throw new KonyError(6001, "Invalid shapesData", "shapesData does not have mandatory value points for shapeType line.");
        ...
        if(shapesDataArr && shapesDataArr.length > 0) {
            for(i = 0; i < shapesDataArr.length; ++i) {
                 if(existingDataObj[shapesDataArr[i].shapeid]) {
                    throw new KonyError('CanvasError', 'addShapes:: already shapeid exists');
                 } else {
